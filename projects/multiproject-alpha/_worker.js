...
async function activeProject(env){const file=await github(env,'/contents/factory-state/projects.json?ref=factory-control');const state=JSON.parse(decodeContent(file.content||''));return state.projects?.['multiproject-alpha']||null}
function stageFromSteps(run,jobs){if(!run)return 'idle';if(run.status==='queued')return 'prompt';if(run.status==='completed')return run.conclusion==='success'?'review':'error';const steps=jobs?.jobs?.[0]?.steps||[];const active=steps.find(s=>s.status==='in_progress');if(!active)return 'build';const n=String(active.name||'').toLowerCase();if(n.includes('qa')||n.includes('browser')||n.includes('preview'))return 'qa';if(n.includes('validate preview'))return 'preview';if(n.includes('execute factory')||n.includes('implement')||n.includes('checkout generated'))return 'build';return 'build'}
async function liveStatus(env){
  ...
  return {
    ...
    project_name:project?.project_name||'MultiProject Alpha',
    stage,
    ...
    preview_url:project?.preview_url||'https://factory-multiproject-alpha-e-huml.chatgpt-factory-preview.pages.dev',
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==='/api/factory/request'&&request.method==='POST'){
      ...
      const created=await github(env,`/contents/${file}`,{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({message:`RIOSYSTEMS: ${mode} MultiProject Alpha`,content,branch:'factory-control'})});
      ...
      return json({ok:true,request_file:file,mode,production_deploy:false,commit_sha:created?.commit?.sha||null,content_sha:verified.sha},202);
    }
