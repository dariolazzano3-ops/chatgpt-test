...
create table if not exists public.riosystems_operator_runtime_v1 (
  ...
  constraint riosystems_operator_runtime_schema_check check (runtime ->> 'schema' = 'riosystems.operator-runtime.v1'),
  constraint riosystems_operator_runtime_operator_check check (runtime ->> 'operator_id' = operator_id),
  constraint riosystems_operator_runtime_revision_check check ((runtime ->> 'revision')::bigint = revision)
);
