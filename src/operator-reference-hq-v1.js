...
body .main{max-width:none;width:100%;margin:0;padding:0 18px 30px;min-width:0;background:transparent}
...
.rf-hero:before{content:"";position:absolute;right:-2%;bottom:-17%;width:69%;height:126%;opacity:.92;background:linear-gradient(180deg,#243640 0%,#16262e 42%,#0b171d 100%);clip-path:polygon(0 100%,8% 80%,17% 87%,29% 55%,37% 72%,48% 25%,57% 61%,67% 37%,78% 67%,88% 48%,100% 72%,100% 100%);filter:drop-shadow(0 15px 10px rgba(0,0,0,.38))}
.rf-hero:after{content:"";position:absolute;right:-2%;bottom:-4%;width:65%;height:58%;opacity:.36;background:linear-gradient(180deg,#40515a,#101b20);clip-path:polygon(0 100%,10% 68%,21% 78%,32% 57%,45% 79%,58% 52%,70% 75%,82% 59%,100% 82%,100% 100%)}
...
@media(max-width:1180px){body .app{grid-template-columns:196px 1fr}.rf-bottom{grid-template-columns:1fr}.rf-middle{grid-template-columns:1fr}.rf-middle>.rf-panel,.rf-bottom>.rf-panel{min-height:0}.rf-bottom>.rf-panel:last-child{display:block}.rf-bottom>.rf-panel:last-child .rf-selected-grid{height:auto;grid-template-rows:auto}.rf-bottom>.rf-panel:last-child .rf-mini-grid{height:auto}.rf-kpis{grid-template-columns:1fr 1fr}.rf-nav-spacer{height:220px}}
...
(function(){
  ...
  function portfolio(){
    ...
    var rowHtml=rows.map(function(p){var c=context(p.scope_key),pi=previewOf(p,null,c),label=statusLabel(p,c),chips='<span class="rf-pchip '+chipTone(label)+'">'+H(label)+'</span>';if(pi.available===true)chips+='<span class="rf-pchip green">PREVIEW VERFÜGBAR</span>';if(approvalCount(p,c)>0)chips+='<span class="rf-pchip gold">FREIGABE OFFEN</span>';var initial=String(p.name||p.project_id||'?').trim().charAt(0).toUpperCase(),action=p.project_detail_openable===true?'<button class="rf-open" data-rf-open="'+H(p.scope_key)+'" aria-label="Projekt öffnen">›</button>':p.workspace_enabled===true?'<button class="rf-open" data-rf-workspace="'+H(p.project_workspace_route||('/operator/workspace/'+encodeURIComponent(p.scope_key||'')))+'" aria-label="Workspace öffnen">›</button>':'<span class="rf-open" aria-label="Detail nicht verfügbar">·</span>';return'<div class="rf-project-row '+(sel&&sel.scope_key===p.scope_key?'selected':'')+'" data-rf-row="'+H(p.scope_key)+'"><div class="rf-thumb">'+H(initial)+'</div><div class="rf-project-name"><b>'+H(p.name||p.project_id)+'</b><span>'+H(p.scope_key)+'</span></div><div class="rf-project-chips">'+chips+'</div>'+action+'</div>'}).join('');
  ...
  function bind(){
    ...
    document.querySelectorAll('[data-rf-open]').forEach(function(b){b.onclick=function(){var p=items().find(function(x){return x.scope_key===b.dataset.rfOpen});if(p)openProject(p,'overview')}});document.querySelectorAll('[data-rf-workspace]').forEach(function(b){b.onclick=function(){location.href=b.dataset.rfWorkspace}});
...
export function injectReferenceDrivenHq(html=''){
  ...
  const withFavicon=/<link[^>]+rel=["'](?:shortcut )?icon["']/i.test(html)?html:(html.includes('</head>')?html.replace('</head>','<link rel="icon" href="data:,"></head>'):html);
  return withFavicon.includes('</body>')?withFavicon.replace('</body>',ADDON+'</body>'):withFavicon+ADDON;
}
