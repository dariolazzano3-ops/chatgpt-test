...
(() => {
  ...
  const desiredNav = [
    ...
    ['executions','Ausführungen'],['factories','Factories'],['capabilities','Fähigkeiten'],['providers','Provider'],
  ...
  function renderFunctionalCapabilities() {
    ...
    root.innerHTML='<div class="card"><div class="row"><div><h2>Fähigkeiten</h2><div class="small">Human Labels zuerst. Technische Capability-Codes und dieselbe Universal-Mission-Router-Registry bleiben als Evidence sichtbar.</div></div>'+badge(reg.status||'UNKNOWN')+'</div>'+
      (items.length?'<div class="table-wrap"><table class="table"><thead><tr><th>Fähigkeit</th><th>Factory</th><th>Status</th><th>Primärer Provider</th><th>Fallback</th><th>Voraussetzungen</th></tr></thead><tbody>'+items.map(x=>'<tr><td><strong>'+esc(typeof humanCapability==="function"?humanCapability(x.capability):x.capability)+'</strong><div class="small mono">Technisch: '+esc(x.capability)+'</div><div class="small">'+esc(x.expected_deliverable||'')+'</div></td><td><strong>'+esc(typeof humanFactory==="function"?humanFactory(x.factory):x.factory||'Nicht erfasst')+'</strong><div class="small mono">Technisch: '+esc(x.factory||'UNKNOWN')+'</div></td><td>'+badge(x.status)+'</td><td>'+esc(x.provider_primary||'Nicht verifiziert')+'<div>'+badge(x.provider_status)+'</div></td><td>'+esc(x.provider_fallback||'Kein Fallback projiziert')+'</td><td class="small">'+esc(list(x.requirements).join(', ')||'Keine zusätzliche Voraussetzung für die synthetische Route')+'</td></tr>').join('')+'</tbody></table></div>':noData('Fähigkeiten konnten nicht aus dem Core projiziert werden.'))+'</div>';
  }
  ...
  function renderFunctionalProviders() {
    ...
    card.innerHTML='<h2>Normalized Provider Control</h2><div class="small">Credentials werden nie angezeigt. Fehlende Live-Evidence bleibt NOT VERIFIED.</div>'+
      (items.length?items.map(x=>'<div class="row"><div class="row-main"><strong>'+esc(x.name)+'</strong><div class="small">'+esc(x.category||'provider')+' · '+esc(x.environment||'UNKNOWN')+' · credentials '+esc(x.credentials_state||'NOT_VERIFIED')+' · cost '+esc(x.cost_mode||'NOT_VERIFIED')+'</div><div class="small">Fähigkeiten: '+esc(list(x.capabilities).map(v=>typeof humanCapability==="function"?humanCapability(v):v).join(', ')||'nicht projiziert')+' · Einschränkungen: '+esc(list(x.current_restrictions).join(', ')||'keine projiziert')+'</div></div>'+badge(x.status)+'</div>').join(''):noData('Keine Provider-Evidence projiziert.'));
  ...
  function renderFunctionalCosts() {
    ...
    card.innerHTML='<h2>Kostenkontrolle</h2><div class="grid three"><div class="kv"><b>Tageskosten</b><span>'+esc(maybeMoney(c.daily_cost_eur,c.daily_cost_state))+'</span><div class="small mono">Technisch: '+esc(c.daily_cost_state||'UNKNOWN')+'</div></div><div class="kv"><b>Monatskosten</b><span>'+esc(maybeMoney(c.monthly_cost_eur,c.monthly_cost_state))+'</span><div class="small mono">Technisch: '+esc(c.monthly_cost_state||'UNKNOWN')+'</div></div><div class="kv"><b>Variables Missionslimit</b><span>'+esc(maybeMoney(c.mission_variable_budget_ceiling_eur,c.mission_variable_budget_state||'RESERVED'))+'</span></div></div><div class="row"><span>Development-Limit</span><strong>'+esc(c.development_ceiling_eur===null||c.development_ceiling_eur===undefined?'Nicht konfiguriert':fmtMoney(c.development_ceiling_eur))+'</strong></div><div class="row"><span>Verbleibendes Development-Budget</span><strong>'+esc(c.remaining_development_budget_eur===null||c.remaining_development_budget_eur===undefined?'Nicht verifiziert':fmtMoney(c.remaining_development_budget_eur))+'</strong></div><div class="row"><span>Budget-blockierte Events</span><strong>'+esc(list(c.blocked_by_budget_events).length)+'</strong></div><p class="small">Actual, Estimated, Reserved, Unknown und Not Reconciled bleiben getrennt. Fehlende Perioden-Evidence wird niemals zu verifizierten 0 €.</p>';
    root.appendChild(card);
  }
