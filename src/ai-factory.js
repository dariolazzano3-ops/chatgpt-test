...
const AI_TASK_TYPES = new Set([
  'analyze', 'structure', 'classify', 'extract', 'summarize', 'generate', 'decision_support'
]);
...
export function aiFactoryManifest() {
  return {
    id: 'ai-factory-v1', version: '1.1.0', contract_version: 'ai.task.v1', result_contract_version: 'ai.result.v1',
    status: 'structured_output_foundation', available: true, execution_mode: 'contract_only', automatic_execution: false,
    provider_agnostic: true, model_routing: false, structured_output_validation: true, schema_contracts: true,
    tool_access: false, external_data_access: false, external_side_effects: false, production_deploy: false,
    supported_task_types: [...AI_TASK_TYPES], supported_output_formats: [...OUTPUT_FORMATS],
  ...
  if (!isObject(output)) errors.push('AI_OUTPUT_CONTRACT_REQUIRED');
    ...
    if (format === 'structured_json') {
      if (!isObject(output.schema)) errors.push('AI_OUTPUT_SCHEMA_REQUIRED');
        ...
        const schemaValidation = validateStructuredOutputSchema(output.schema);
        for (const error of schemaValidation.errors) errors.push(`${error.code}:${error.path}`);
      }
    }
...
export function normalizeAIContract(contract = {}) {
  ...
  return { ok: true, contract: {
    contract_version: 'ai.task.v1', task_type: validation.task_type, goal: clean(contract.goal, 4000), input: cloneJson(contract.input), context,
    output: { format, schema: format === 'structured_json' ? cloneJson(contract.output.schema) : null, max_chars: Number(contract.output.max_chars ?? MAX_OUTPUT_CHARS) },
    execution: { max_attempts: Number(contract.execution?.max_attempts ?? 1), allow_tools: false, allow_external_data: false, automatic: false, production_deploy: false },
...
export function normalizeAIResultContract(result = {}) {
  ...
  return { ok: true, result: {
    result_contract_version: 'ai.result.v1', status: validation.status,
    ...
    provider: clean(result.provider, 120) || null, model: clean(result.model, 160) || null,
    attempts: Number.isInteger(result.attempts) && result.attempts >= 0 ? result.attempts : 0,
    production_deploy: false, external_side_effects: false
  }};
}
