...
async function handleWebsite(request, service, options = {}) {
  ...
  const read = await loadIntake(service, body.scope_key);
  if (!read.ok) return json(read.body, read.status || 400);
  const referenceOnly = body.reference_only === true || clean(body.source_type, 80).toUpperCase() === 'REFERENCE_WEBSITE';
...
async function handleWebsiteUsage(request, service) {
  ...
  const read = await loadIntake(service, body.scope_key);
  if (!read.ok) return json(read.body, read.status || 400);
  const updated = updateProjectSourceWebsiteUsage(read.body.state, body.source_id, {
    scope_key: clean(body.scope_key, 640),
    website_usage: body.website_usage
  });
  if (!updated.ok) return json(updated, updated.error === 'PROJECT_SOURCE_NOT_FOUND' ? 404 : 400);
  const saved = await saveIntake(service, read, updated.state, 'PROJECT_SOURCE_WEBSITE_USAGE_UPDATED');
  if (!saved.ok) return json(saved.body, saved.status || 409);
  const effective = effectiveProjectWebsiteUsage(updated.source);
  return json({ ok: true, source: { ...updated.source, rights_status: effective.rights_status, effective_usage: effective.effective_usage }, runtime_revision: saved.body.runtime_revision, variable_cost_eur: 0, paid_provider_calls: 0, production_deploy: false });
}
...
export async function handleOperatorDashboard(request, env = {}, ctx = {}, options = {}) {
  ...
  if (url.pathname === '/operator/api/project-source-intake/website-usage' && request.method === 'PATCH') return handleWebsiteUsage(request, service);
