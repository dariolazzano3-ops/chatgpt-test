...
export function buildBakeryMullerBlock6Delivery(evidence = {}) {
  ...
  const checks = {
    web: web.ok === true && web.staging === true,
    make: make.ok === true && Number(make.scenario_id) === 7149691 && Boolean(make.execution_id) && make.scenario_restored_inactive === true,
    supabase: supabase.ok === true && supabase.lead_count === 1 && supabase.event_count === 1 && supabase.provider_ref_count === 1 && supabase.audit_count === 1,
    posthog: posthog.ok === true && posthog.event_count === 5 && posthog.flow_id === FLOW_ID,
    ai: ai.ok === true && ai.http_status === 200 && ai.api_success === true && Number(ai.neurons) > 0 && Number(ai.variable_cost_eur) === 0
  };
