...
(() => {
  ...
  const providerLabels={
    'cloudflare-workers-ai-free':'Workers AI',
    'cloudflare-workers-free':'Cloudflare',
    'openai-api':'OpenAI',
    'posthog-free':'PostHog',
    'supabase-free':'Supabase'
  };
  const costLabels={Spent:'Ausgegeben',Reserved:'Reserviert',Estimated:'Geschätzt',Remaining:'Verbleibend'};
  ...
  function humanizeProviderLabels(){const root=document.getElementById('providers');if(!root)return;root.querySelectorAll('.human-card h3').forEach(title=>{if(title.dataset.providerLabel==='true')return;const raw=(title.textContent||'').trim();const label=providerLabels[raw.toLowerCase()];if(!label)return;title.dataset.providerLabel='true';title.textContent=label;const meta=document.createElement('div');meta.className='human-meta';meta.textContent='Provider ID: '+raw;title.insertAdjacentElement('afterend',meta)})}
  function humanizeCostLabels(){const root=document.getElementById('costs');if(!root)return;root.querySelectorAll('.metric .k').forEach(el=>{const raw=(el.textContent||'').trim();if(costLabels[raw])el.textContent=costLabels[raw]})}
  ...
  function finalPolish(id){if(id==='projects'){wrapProjectCreate();clearStaleProjectPriority()}if(id==='providers')humanizeProviderLabels();if(id==='costs')humanizeCostLabels();if(id==='audit')humanize(document.getElementById('audit'));if(state?.detail&&document.getElementById('project-detail')?.children.length)humanProjectDetail(state.detail)}
