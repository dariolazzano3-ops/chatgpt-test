...
.human-input-closure-v1{margin-top:14px}.human-input-summary{display:flex;gap:9px;align-items:center;flex-wrap:wrap}.human-input-sprint{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;margin-top:12px;padding:12px;border:1px solid #e6d4a5;border-radius:12px;background:#fff9e9}.human-input-sprint strong{display:block;margin-bottom:3px}.human-input-sprint .btn{white-space:nowrap}.human-input-question{border:1px solid var(--line);border-radius:12px;padding:13px;margin-top:10px;background:#fff}.human-input-question h3{margin:0 0 6px}.human-input-controls{display:grid;gap:10px;margin-top:11px}.human-input-control{border-top:1px solid var(--soft);padding-top:9px}.human-input-control:first-child{border-top:0;padding-top:0}.human-input-options{display:grid;gap:7px;margin-top:7px}.human-input-option{display:flex;gap:8px;align-items:flex-start}.human-input-option input{margin-top:3px}.human-input-other{margin-top:7px;width:100%}.human-input-actions{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:12px}.human-input-status{font-size:12px}.human-research-evidence{margin-top:10px;padding:10px;border-radius:10px;background:var(--soft);display:grid;gap:6px;min-width:0;max-width:100%;overflow:hidden}.human-research-row{display:grid;gap:3px;min-width:0;max-width:100%;overflow-wrap:anywhere;word-break:break-word}.human-research-row *{min-width:0;max-width:100%;overflow-wrap:anywhere;word-break:break-word}.human-research-sources{display:flex;gap:5px;flex-wrap:wrap;min-width:0;max-width:100%}.human-research-source{font-size:11px;padding:3px 6px;border:1px solid var(--line);border-radius:999px;background:#fff;max-width:100%;overflow-wrap:anywhere;word-break:break-word}.human-input-resolved{margin-top:10px;padding-top:9px;border-top:1px solid var(--soft)}@media(max-width:760px){.human-input-sprint{grid-template-columns:1fr}.human-input-sprint .btn{width:100%;min-height:46px}.human-input-question{padding:14px}.human-input-actions .btn{width:100%;min-height:46px}.human-input-control select,.human-input-control input[type=text]{width:100%;min-height:44px}}
...
return controls};
const researchEvidence=q=>{const m=q?.multi_source_verification;if(!m)return '';const rows=Array.isArray(m.evidence)?m.evidence:[];const rowHtml=rows.map(r=>'<div class="human-research-row"><div><b>'+e(r.field_path||'Evidence')+'</b> · '+e(r.classification||'')+(r.conflict?' · ⚠️ Konflikt':'')+'</div>'+(Array.isArray(r.candidates)?r.candidates.map(c=>'<div class="small">Candidate: <b>'+e(typeof c.value==='string'?c.value:JSON.stringify(c.value))+'</b> · '+e(c.independent_source_count||0)+' Quellen'+(c.freshness?.state?' · '+e(c.freshness.state):'')+'</div><div class="human-research-sources">'+(Array.isArray(c.sources)?c.sources.map(s=>'<span class="human-research-source">'+e(s.source_role||s.source_id||'Source')+'</span>').join(''):'')+'</div>').join(''):'')+'</div>').join('');return '<div class="human-research-evidence"><div><b>Multi-Source Evidence:</b> '+e(m.research_state||'')+'</div>'+rowHtml+'<div class="small">'+e(m.confirmation_reason||'')+'</div></div>'};
const workspaceFor=root=>root.closest('.pm-workspace')||document.querySelector('.pm-workspace[data-j12-action]')||document.querySelector('.pm-workspace');
const j12ActionFor=root=>window.__aurentaraJ12CurrentAction?.action||workspaceFor(root)?.dataset?.j12Action||'';
const priorityQuestion=(root,items)=>{const code=j12ActionFor(root),preferred=items.find(q=>q.id===code);return preferred||items[0]||null};
const focusNext=(root,preferred='')=>{const card=root.querySelector('[data-human-input-closure]');if(!card)return null;const rows=[...card.querySelectorAll('[data-human-question]')];let target=preferred?card.querySelector('[data-human-question="'+CSS.escape(preferred)+'"]'):null;if(!target){const code=j12ActionFor(root);if(code)target=card.querySelector('[data-human-question="'+CSS.escape(code)+'"]')}if(!target)target=rows[0]||null;for(const row of rows)row.dataset.humanCurrent=row===target?'true':'false';if(target){target.scrollIntoView({block:'center',behavior:'smooth'});const control=target.querySelector('input,select,textarea,button');if(control)control.focus({preventScroll:true})}return target?.dataset?.humanQuestion||null};
const sprintMarkup=(root,items,resolved)=>{const next=priorityQuestion(root,items);if(!next)return '';const j12=j12ActionFor(root)===next.id;return '<div class="human-input-sprint" data-human-decision-sprint data-human-priority="'+e(next.id)+'"><div><div class="eyebrow">'+(j12?'NEXT BEST ACTION · J12':'DECISION SPRINT')+'</div><strong>Nächste Entscheidung: '+e(next.question||next.id)+'</strong><div class="small">'+e(items.length)+' offen · '+e(resolved.length)+' erledigt. Keine Entscheidung wird automatisch bestätigt.</div></div><button class="btn primary" data-human-next>Jetzt entscheiden</button></div>'};
const render=(root,payload)=>{const closure=payload?.human_input_closure||{},items=closure.open_inputs||[],resolved=closure.resolved_inputs||[];root.__humanClosurePayload=payload;let card=root.querySelector('[data-human-input-closure]');if(!card){card=document.createElement('div');card.className='card human-input-closure-v1';card.dataset.humanInputClosure='true';root.prepend(card)}card.__startedAt=new Date().toISOString();card.innerHTML='<div class="eyebrow">Project Knowledge</div><div class="human-input-summary"><h2 style="margin:0">OPEN INPUTS / DECISIONS</h2><span class="badge '+(items.length?'attention':'ready')+'">OPEN INPUTS: '+e(items.length)+'</span></div>'+sprintMarkup(root,items,resolved)+(items.length?items.map(q=>'<div class="human-input-question" data-human-question="'+e(q.id)+'"><h3>'+e(q.question)+'</h3><div class="small"><b>Warum benötigt:</b> '+e(q.reason||'Human confirmation required')+'</div>'+researchEvidence(q)+'<div class="human-input-controls">'+(q.controls||[]).map(c=>renderControl(q,c)).join('')+'</div><div class="human-input-actions"><button class="btn primary" data-human-save="'+e(q.id)+'">Confirm / Save</button><span class="human-input-status" data-human-status="'+e(q.id)+'"></span></div></div>').join(''):'<div class="callout good" style="margin-top:12px"><strong>OPEN INPUTS = 0</strong><div class="small">Alle Human Decisions dieses Projekts wurden beantwortet. Bestehende Readiness- und Quality-Gates bleiben weiterhin autoritativ.</div></div>')+(resolved.length?'<div class="human-input-resolved small"><b>RESOLVED: '+e(resolved.length)+'</b> · '+resolved.map(x=>e(x.id)).join(', ')+'</div>':'');const next=card.querySelector('[data-human-next]');if(next)next.onclick=()=>focusNext(root);for(const q of items){const qcard=card.querySelector('[data-human-question="'+CSS.escape(q.id)+'"]'),button=qcard?.querySelector('[data-human-save="'+CSS.escape(q.id)+'"]'),status=qcard?.querySelector('[data-human-status="'+CSS.escape(q.id)+'"]');if(!button)continue;button.onclick=async()=>{button.disabled=true;if(status)status.textContent='Speichert…';try{await api('/human-decision',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({scope_key:payload.identity.scope_key,context_scope_key:payload.identity.scope_key,question_id:q.id,controls:collect(qcard,q),interaction_started_at:card.__startedAt})});const fresh=await api('?scope_key='+encodeURIComponent(payload.identity.scope_key));render(root,fresh);setTimeout(()=>focusNext(root),0);if(typeof window.__aurentaraProjectSourceIntakeRefresh==='function')void window.__aurentaraProjectSourceIntakeRefresh(payload.identity.scope_key)}catch(err){if(status)status.textContent='⚠️ '+String(err?.data?.error||err.message||err);if(typeof window.setError==='function')window.setError(err)}finally{button.disabled=false}}}};
...
const applyJ12Handoff=detail=>{const scope=detail?.scope||'',action=detail?.action||'',root=[...document.querySelectorAll('[data-project-source-intake]')].find(x=>!scope||x.dataset.scope===scope)||document.querySelector('[data-project-source-intake]');if(!root)return false;if(root.__humanClosurePayload)render(root,root.__humanClosurePayload);const card=root.querySelector('[data-human-input-closure]'),target=action?card?.querySelector('[data-human-question="'+CSS.escape(action)+'"]'):null,sprint=card?.querySelector('[data-human-decision-sprint]');if(!target||!sprint)return false;sprint.dataset.humanPriority=action;const eyebrow=sprint.querySelector('.eyebrow'),strong=sprint.querySelector('strong');if(eyebrow)eyebrow.textContent='NEXT BEST ACTION · J12';if(strong)strong.textContent='Nächste Entscheidung: '+(target.querySelector('h3')?.textContent||action);return true};
window.__aurentaraHumanInputClosureApplyJ12=applyJ12Handoff;
document.addEventListener('aurentara:j12-next-best-action',event=>applyJ12Handoff(event?.detail||{}));
})();</script>`;
}
...
export function dashboardHumanInputClosureManifest() {
  return {
    ...
    multi_source_evidence_in_existing_open_inputs: true,
    canonical_human_questions_reused: true,
    cross_project_mutation_blocked: true,
    ai_auto_confirmation: false,
    desktop_and_mobile_controls: true,
    j12_priority_handoff: true,
    j12_direct_handoff_hook: true,
    decision_sprint_next_navigation: true,
    save_advances_to_next_open_input: true,
    production_deploy: false,
    public_launch: false,
    dns_change: false,
