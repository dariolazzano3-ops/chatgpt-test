...
else if(type==='SINGLE_CHOICE'){body='<select data-human-single="'+key+'"><option value="">Bitte auswählen</option>'+items.map((x,i)=>'<option value="'+i+'">'+e(label(x))+'</option>').join('')+(c.allow_other?'<option value="other">Andere Angabe</option>':'')+'</select>'+(c.allow_other?'<input class="human-input-other" type="text" data-human-other="'+key+'" placeholder="Andere Angabe">':'');}
...
else if(type==='SINGLE_CHOICE'){const select=card.querySelector('[data-human-single="'+CSS.escape(key)+'"]'),v=select?.value||'',items=Array.isArray(c.candidates)?c.candidates:[];controls[c.id]=v==='other'?{value:null,other_value:card.querySelector('[data-human-other="'+CSS.escape(key)+'"]')?.value?.trim()||''}:v===''?{}:{value:candidate(items[Number(v)])};}
