...
document.addEventListener('aurentara:j12-next-best-action',event=>{const scope=event?.detail?.scope||'',action=event?.detail?.action||'',root=[...document.querySelectorAll('[data-project-source-intake]')].find(x=>!scope||x.dataset.scope===scope)||document.querySelector('[data-project-source-intake]');if(root?.__humanClosurePayload)render(root,root.__humanClosurePayload);const card=root?.querySelector('[data-human-input-closure]'),target=action?card?.querySelector('[data-human-question="'+CSS.escape(action)+'"]'):null,sprint=card?.querySelector('[data-human-decision-sprint]');if(target&&sprint){sprint.dataset.humanPriority=action;const eyebrow=sprint.querySelector('.eyebrow'),strong=sprint.querySelector('strong');if(eyebrow)eyebrow.textContent='NEXT BEST ACTION · J12';if(strong)strong.textContent='Nächste Entscheidung: '+(target.querySelector('h3')?.textContent||action)}});
})();</script>`;
}
