...
const j12ActionFor=root=>{const scope=root?.dataset?.scope||root?.__humanClosurePayload?.identity?.scope_key||workspaceFor(root)?.dataset?.scope||'',live=window.__aurentaraJ12CurrentAction;if(live?.action&&(!scope||!live.scope||live.scope===scope))return live.action;return workspaceFor(root)?.dataset?.j12Action||''};
...
document.addEventListener('aurentara:j12-next-best-action',event=>{const scope=event?.detail?.scope||'',root=[...document.querySelectorAll('[data-project-source-intake]')].find(x=>!scope||x.dataset.scope===scope)||document.querySelector('[data-project-source-intake]');if(root?.__humanClosurePayload)render(root,root.__humanClosurePayload)});
})();</script>`;
}
