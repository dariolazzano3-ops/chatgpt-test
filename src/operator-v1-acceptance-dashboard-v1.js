...
async function acceptanceResponse(request, env, ctx, options = {}) {
  ...
  const deployedSha = String(env.RIOSYSTEMS_DEPLOYED_FACTORY_CONTROL_SHA || '').trim() || null;
  const acceptance = buildRiosystemsV1AcceptanceFromEvidence({
    ...
    deployment_evidence: deployedSha ? { deployed_sha: deployedSha } : {}
  });
  return json({
    ...acceptance,
    checked_at: health.checked_at || new Date().toISOString(),
    canonical_head_sha: health.branch_truth?.head_sha || null,
