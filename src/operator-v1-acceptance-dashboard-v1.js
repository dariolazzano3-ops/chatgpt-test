...
import { readAuthoritativeCloudflareAccessEvidence } from './operator-cloudflare-access-evidence-v1.js';
...
async function acceptanceResponse(request, env, ctx, options = {}) {
  ...
  const owner = options.system_health_owner || 'dariolazzano3-ops';
  const repo = options.system_health_repo || 'chatgpt-test';
  const branch = options.system_health_branch || 'factory-control';

  const [deploymentEvidence, accessEvidence] = await Promise.all([
    readAuthoritativeStagingDeploymentEvidence({
    ...
    readAuthoritativeCloudflareAccessEvidence({
      fetch_impl: options.cloudflare_access_evidence_fetch || globalThis.fetch,
      owner,
      repo,
      branch,
      canonical_head_sha: canonicalHead
  ...
  return json({
    ...
    cloudflare_access_evidence: accessEvidence,
...
(() => {
  ...
  async function renderAcceptance() {
    ...
    try {
      ...
      const access = result.cloudflare_access_evidence || {};
      ...
      card.innerHTML = '<div class="row"><div><div class="eyebrow">Formal Definition of Done</div><h2 style="margin:3px 0">RIOSYSTEMS V1 Acceptance</h2><div class="small">Code-Evidence + Live-Staging-Evidence · fail-closed</div></div>' + badge(result.status) + '</div>' +
        ...
        '<div class="row"><div><strong>Private Cloudflare Access</strong><div class="small">' + esc(access.label || 'Not verified') + '</div></div>' + badge(access.status || 'NOT_VERIFIED') + '</div>' +
...
export function operatorV1AcceptanceDashboardManifest() {
  return {
    ...
    access_source: 'github_actions_cloudflare_access_readonly',
    access_success_not_inferred: true,
    access_exact_head_required: true,
