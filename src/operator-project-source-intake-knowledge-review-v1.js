...
function renderItem(item,scope,status){const editable=status!=='APPROVED';const val=item.type==='FACT'?fmt(item.value):(item.type==='SOURCE'?(item.source_type||'Quelle'):(item.usage_role||'Bild'));const conflict=item.type==='FACT'&&item.verification_status==='SOURCE_CONFLICT';return '<div class="knowledge-item" data-knowledge-item="'+esc(item.type)+':'+esc(item.id)+'"><div class="knowledge-item-row"><div><strong>'+esc(item.label||item.id)+'</strong><div class="knowledge-item-value">'+esc(val)+'</div><div class="knowledge-item-meta">'+esc(item.type+(item.field_path?' · '+item.field_path:'')+(item.verification_status?' · '+item.verification_status:''))+'</div></div>'+(editable?'<div class="source-tools">'+(conflict?'<button class="btn primary" data-knowledge-conflict-confirm="'+esc(item.id)+'">Diese Angabe bestätigen</button><button class="btn" data-knowledge-conflict-reject="'+esc(item.id)+'">Ignorieren</button>':'')+'<button class="btn" data-knowledge-edit="'+esc(item.type)+':'+esc(item.id)+'">Bearbeiten</button></div>':'')+'</div></div>'}
