...
.knowledge-catch-net{margin-top:12px;padding:12px;border:1px solid #f2c48d;border-radius:12px;background:#fff7ed}
.knowledge-catch-net.clear{border-color:#86d6aa;background:#ecfdf3}
.knowledge-catch-net-grid{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}
.knowledge-catch-chip{font-size:11px;padding:5px 8px;border-radius:999px;background:#fff;border:1px solid var(--line)}
.knowledge-flag{display:inline-flex;margin:4px 5px 0 0;padding:3px 6px;border-radius:999px;background:#fff7ed;border:1px solid #f2c48d;font-size:10px;font-weight:650}
.knowledge-pot{margin-top:14px;padding:14px;border:1px solid #9db9e8;border-radius:12px;background:#f3f7ff}
.knowledge-pot strong{display:block;margin-bottom:4px}
...
const stateText=status=>({NOT_STARTED:'Noch nicht aufbereitet',COLLECTING:'Rohquellen werden gesammelt',IN_REVIEW:'Informationen sind aufbereitet und warten auf Prüfung',CHANGES_PENDING:'Neue Änderungen vorhanden. Der Projektwissen-Entwurf ist nicht mehr aktuell.',STAGED:'Projektwissen-Entwurf ist gefüllt. Ferrari bleibt noch gesperrt.',APPROVED:'Projektwissen ist bereit'}[status]||status);
const errorText=e=>({PROJECT_KNOWLEDGE_CATCH_NET_REVIEW_REQUIRED:'Im Auffangnetz liegen noch Angaben, die du einzeln prüfen, bestätigen, bearbeiten oder ignorieren musst.',PROJECT_KNOWLEDGE_STAGE_EXPLICIT_CONFIRMATION_REQUIRED:'Bitte bestätige zuerst, dass du die geprüften Informationen in den Projektwissen-Entwurf übernehmen möchtest.',PROJECT_KNOWLEDGE_MUST_BE_STAGED_BEFORE_APPROVAL:'Bitte übernimm die geprüften Informationen zuerst in den Projektwissen-Entwurf.',PROJECT_KNOWLEDGE_STAGE_REQUIRED:'Der Projektwissen-Entwurf fehlt. Bitte zuerst die geprüften Informationen übernehmen.',PROJECT_KNOWLEDGE_STAGE_STALE:'Der Projektwissen-Entwurf ist veraltet. Bitte die Änderungen erneut prüfen.',PROJECT_KNOWLEDGE_EXPLICIT_APPROVAL_REQUIRED:'Bitte bestätige den Projektwissen-Entwurf vor dem Bereitstellen.',PROJECT_KNOWLEDGE_REVIEW_NOT_PREPARED:'Bitte zuerst mit KI aufbereiten.',PROJECT_KNOWLEDGE_REVIEW_EMPTY:'Ohne Rohmaterial kann kein Projektwissen erstellt werden.',PROJECT_KNOWLEDGE_REVIEW_REOPEN_REQUIRED:'Bitte den Projektwissen-Entwurf zuerst wieder zur Bearbeitung öffnen.',PROJECT_IMAGE_VISION_BATCH_LIMIT_EXCEEDED:'Bitte maximal 6 Bilder gleichzeitig zum Informationen-Auslesen markieren.',PROJECT_IMAGE_VISION_STORAGE_READ_FAILED:'Ein markiertes Bild konnte nicht sicher aus dem privaten Speicher geladen werden.',PROJECT_IMAGE_VISION_OPENAI_CREDENTIAL_REQUIRED:'Die Bildanalyse ist im Staging gerade nicht verfügbar.',PROJECT_IMAGE_VISION_REAL_INFERENCE_NOT_ENABLED:'Die Bildanalyse ist im Staging gerade nicht aktiviert.',PROJECT_IMAGE_VISION_STAGING_SAFETY_CONTRACT_NOT_MET:'Die Bildanalyse ist nur im sicheren privaten Staging erlaubt.',PROJECT_IMAGE_VISION_COST_CEILING_EXCEEDED:'Die Bildanalyse wurde durch das Kostenlimit gestoppt.',PROJECT_TEXT_EXTRACTION_BATCH_LIMIT_EXCEEDED:'Es sind zu viele Textquellen für einen sicheren Aufbereitungslauf markiert.',PROJECT_TEXT_EXTRACTION_OPENAI_CREDENTIAL_REQUIRED:'Die Textanalyse ist im Staging gerade nicht verfügbar.',PROJECT_TEXT_EXTRACTION_REAL_INFERENCE_NOT_ENABLED:'Die Textanalyse ist im Staging gerade nicht aktiviert.',PROJECT_TEXT_EXTRACTION_STAGING_SAFETY_CONTRACT_NOT_MET:'Die Textanalyse ist nur im sicheren privaten Staging erlaubt.',PROJECT_TEXT_EXTRACTION_COST_CEILING_EXCEEDED:'Die Textanalyse wurde durch das Kostenlimit gestoppt.',PROJECT_TEXT_EXTRACTION_TOTAL_COST_CEILING_EXCEEDED:'Die Textanalyse wurde durch das Gesamt-Kostenlimit gestoppt.'}[e?.data?.error||e?.message]||('Aktion fehlgeschlagen: '+(e?.data?.error||e?.message||'UNKNOWN')));
const flagText=flag=>({SOURCE_CONFLICT:'Widerspruch',LOW_CONFIDENCE:'Unsicher erkannt',CATEGORY_UNCERTAIN:'Kategorie unsicher',CATEGORY_MISMATCH:'Kategorie prüfen',OPEN_QUESTION:'Offene Frage',UNCATEGORIZED:'Nicht sauber zugeordnet',STALE_SOURCE_EVIDENCE:'Quelle geändert',SOURCE_MISSING:'Quelle fehlt',EXTRACTION_REVIEW_REQUIRED:'Prüfung erforderlich'}[flag]||flag);
...
function renderCatchNet(view){const net=view?.catch_net;if(!net||view?.status==='NOT_STARTED')return '';const c=net.counts||{};const chips=[['Widersprüche',c.source_conflicts],['Unsicher',c.low_confidence],['Kategorie',c.category_uncertain],['Offene Fragen',c.open_questions],['Nicht zugeordnet',c.uncategorized],['Quelle prüfen',c.stale_or_missing_source]].filter(x=>Number(x[1]||0)>0);return '<div class="knowledge-catch-net '+(net.clear?'clear':'')+'"><strong>'+(net.clear?'✅ Auffangnetz ist sauber':'⚠️ Auffangnetz · '+esc(net.unresolved_count||0)+' Angaben brauchen Entscheidung')+'</strong><div class="small">'+(net.clear?'Keine unsicheren, widersprüchlichen oder ungeklärten Angaben blockieren den nächsten Schritt.':'Diese Angaben werden nicht stillschweigend in das Projektwissen übernommen.')+'</div>'+(chips.length?'<div class="knowledge-catch-net-grid">'+chips.map(x=>'<span class="knowledge-catch-chip">'+esc(x[0])+': '+esc(x[1])+'</span>').join('')+'</div>':'')+'</div>'}
function renderItem(item,scope,status){const editable=!['STAGED','APPROVED'].includes(status);const val=item.type==='FACT'?fmt(item.value):(item.type==='SOURCE'?(item.source_type||'Quelle'):(item.usage_role||'Bild'));const fact=item.type==='FACT';const catchInfo=fact?item.catch_net:null;const needsDecision=Boolean(catchInfo?.blocking);const conflict=fact&&item.verification_status==='SOURCE_CONFLICT';const origin=(item.source_refs||[]).length?' · Quellen: '+item.source_refs.join(', '):(item.source_id?' · Quelle: '+item.source_id:'');const confidence=fact&&Number.isFinite(Number(item.confidence))?' · Sicherheit '+Math.round(Number(item.confidence)*100)+'%':'';const flags=catchInfo?.flags?.length?'<div>'+catchInfo.flags.map(flag=>'<span class="knowledge-flag">'+esc(flagText(flag))+'</span>').join('')+'</div>':'';return '<div class="knowledge-item" data-knowledge-item="'+esc(item.type)+':'+esc(item.id)+'"><div class="knowledge-item-row"><div><strong>'+esc(item.label||item.id)+'</strong><div class="knowledge-item-value">'+esc(val)+'</div><div class="knowledge-item-meta">'+esc(item.type+(item.field_path?' · '+item.field_path:'')+(item.verification_status?' · '+item.verification_status:'')+confidence+origin)+'</div>'+flags+'</div>'+(editable?'<div class="source-tools">'+(fact&&needsDecision?'<button class="btn primary" data-knowledge-fact-confirm="'+esc(item.id)+'">'+(conflict?'Diese Angabe bestätigen':'Verwenden')+'</button>':'')+(fact?'<button class="btn" data-knowledge-fact-reject="'+esc(item.id)+'">Ignorieren</button>':'')+'<button class="btn" data-knowledge-edit="'+esc(item.type)+':'+esc(item.id)+'">Bearbeiten</button></div>':'')+'</div></div>'}
function render(root,payload){const panel=ensurePanel(root);const view=payload?.workspace?.knowledge_review||payload?.workspace?.sections?.knowledge_review||payload?.knowledge_review||payload?.review;const status=view?.status||'NOT_STARTED';const gate=view?.gate||{};const stages=view?.stages||[];const net=view?.catch_net||{};panel.innerHTML='<div class="knowledge-review-head"><div><div class="eyebrow">PROJECT FERRARI · INTELLIGENT INTAKE</div><h3>Vom Wäschekorb zum geprüften Projektwissen</h3><div class="small">Alles sammeln → KI extrahiert und sortiert → Auffangnetz prüfen → Projektwissen-Entwurf → bereitstellen.</div></div><span class="knowledge-ai-badge">'+esc(view?.organized_by==='AI'?'🧠 KI aufbereitet':view?.organized_by?'Strukturiert':'Noch roh')+'</span></div>'+
'<div class="knowledge-review-stages">'+stages.map(s=>'<div class="knowledge-stage '+(s.complete?'complete':'')+'">'+esc(s.label)+'</div>').join('')+'</div>'+
'<div class="knowledge-review-message '+(status==='APPROVED'?'approved':gate.allowed===false?'locked':'')+'"><strong>'+esc(stateText(status))+'</strong><div>'+(status==='APPROVED'?'✅ Ferrari darf ausschließlich mit diesem bereitgestellten Wissensstand arbeiten.':status==='STAGED'?'🍲 Die geprüften Informationen liegen im Projektwissen-Entwurf. Der finale Bereitstellen-Schritt ist noch ausstehend.':status==='IN_REVIEW'||status==='CHANGES_PENDING'?'🔒 Ferrari bleibt gesperrt. Erst prüfen, Auffangnetz leeren und die Informationen in den Projektwissen-Entwurf übernehmen.':'Wirf zuerst Websites, Textdateien, Preislisten, Bilder und Notizen in den Wäschekorb. Danach bereitet die KI sie auf.')+'</div></div>'+
renderCatchNet(view)+
...
'<div class="knowledge-review-actions">'+(['APPROVED','STAGED'].includes(status)?'<button class="btn" data-knowledge-reopen>Bearbeitung wieder öffnen</button>':'<button class="btn primary" data-knowledge-prepare>'+(status==='IN_REVIEW'||status==='CHANGES_PENDING'?'Mit KI neu aufbereiten':'Mit KI aufbereiten')+'</button>')+'</div>'+
((status==='IN_REVIEW'||status==='CHANGES_PENDING'||status==='COLLECTING')?'<div class="knowledge-approval"><label><input type="checkbox" data-knowledge-stage-confirm '+(net.clear?'':'disabled')+'> <span>Ich habe die aufbereiteten Informationen geprüft. <strong>Die nicht ignorierten Angaben sollen jetzt in den Projektwissen-Entwurf übernommen werden.</strong></span></label><button class="btn primary" data-knowledge-stage disabled style="margin-top:10px">Diese Informationen übernehmen</button>'+(net.clear?'':'<div class="small" style="margin-top:8px">Zuerst die markierten Angaben im Auffangnetz entscheiden.</div>')+'</div>':'')+
(status==='STAGED'?'<div class="knowledge-pot"><strong>Projektwissen-Entwurf bereit</strong><div class="small">Der Kochtopf ist gefüllt, aber noch nicht angestellt. Ferrari bleibt gesperrt, bis du diesen Wissensstand ausdrücklich bereitstellst.</div><label style="display:flex;gap:9px;align-items:flex-start;margin-top:12px;font-size:13px"><input type="checkbox" data-knowledge-ready-confirm> <span>Ich habe den Projektwissen-Entwurf geprüft und möchte genau diesen Stand bereitstellen.</span></label><button class="btn primary" data-knowledge-approve disabled style="margin-top:10px">Projektwissen bereitstellen</button></div>':'');
const scope=rootScope(root);const prepare=panel.querySelector('[data-knowledge-prepare]');if(prepare)prepare.onclick=async()=>{if(!scope)return;prepare.disabled=true;prepare.textContent='KI bereitet auf…';try{await api('prepare',{scope_key:scope,context_scope_key:scope,allow_ai:true});await refresh(root,scope)}catch(e){alert(errorText(e))}finally{prepare.disabled=false}};
const stageCheck=panel.querySelector('[data-knowledge-stage-confirm]'),stageBtn=panel.querySelector('[data-knowledge-stage]');if(stageCheck&&stageBtn){stageCheck.onchange=()=>stageBtn.disabled=!stageCheck.checked;stageBtn.onclick=async()=>{stageBtn.disabled=true;stageBtn.textContent='Wird übernommen…';try{await api('stage',{scope_key:scope,context_scope_key:scope,review_seen:true,stage_confirmed:true});await refresh(root,scope)}catch(e){alert(errorText(e));stageBtn.disabled=false}}}
const readyCheck=panel.querySelector('[data-knowledge-ready-confirm]'),approve=panel.querySelector('[data-knowledge-approve]');if(readyCheck&&approve){readyCheck.onchange=()=>approve.disabled=!readyCheck.checked;approve.onclick=async()=>{approve.disabled=true;approve.textContent='Projektwissen wird bereitgestellt…';try{await api('approve',{scope_key:scope,context_scope_key:scope,review_seen:true,approval_confirmed:true});await refresh(root,scope)}catch(e){alert(errorText(e));approve.disabled=false}}}
...
export function projectKnowledgeReviewUiManifest() {
  return {
    ...
    explicit_stage_checkbox: true,
    explicit_final_checkbox: true,
    catch_net_visible: true,
    staged_project_knowledge_state: true,
