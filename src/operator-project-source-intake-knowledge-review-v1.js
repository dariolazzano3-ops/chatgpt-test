...
.knowledge-edit{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:7px;margin-top:8px}
...
const errorText=e=>({PROJECT_KNOWLEDGE_CONFLICTS_MUST_BE_RESOLVED:'Es gibt noch widersprüchliche Angaben. Bitte diese zuerst korrigieren.',PROJECT_KNOWLEDGE_EXPLICIT_APPROVAL_REQUIRED:'Bitte die Projektakte zuerst prüfen und den Freigabe-Haken setzen.',PROJECT_KNOWLEDGE_REVIEW_REOPEN_REQUIRED:'Bitte die Freigabe zuerst wieder zur Bearbeitung öffnen.'}[e?.data?.error||e?.message]||('Aktion fehlgeschlagen: '+(e?.data?.error||e?.message||'UNKNOWN')));
...
function renderItem(item,scope,status){const editable=status!=='APPROVED';const val=item.type==='FACT'?fmt(item.value):(item.type==='SOURCE'?(item.source_type||'Quelle'):(item.usage_role||'Bild'));return '<div class="knowledge-item" data-knowledge-item="'+esc(item.type)+':'+esc(item.id)+'"><div class="knowledge-item-row"><div><strong>'+esc(item.label||item.id)+'</strong><div class="knowledge-item-value">'+esc(val)+'</div><div class="knowledge-item-meta">'+esc(item.type+(item.field_path?' · '+item.field_path:'')+(item.verification_status?' · '+item.verification_status:''))+'</div></div>'+(editable?'<button class="btn" data-knowledge-edit="'+esc(item.type)+':'+esc(item.id)+'">Bearbeiten</button>':'')+'</div></div>'}
