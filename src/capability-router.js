...
export function listCapabilities() {
  return Object.entries(CAPABILITIES).map(([id, capability]) => ({ id, ...capability }));
}

export function routeCapability(input = {}) {
  const prompt = text(input.prompt || input.request || input.goal);
  const explicit = text(input.capability);
  const project = text(input.project || input.project_slug);
  ...
  const ranked = [...scores.entries()].map(([id, value]) => ({ id, ...CAPABILITIES[id], ...value })).sort((a, b) => b.score - a.score);
  if (!ranked.length) return { ok: false, error: "CAPABILITY_UNRESOLVED", candidates: [] };

  const top = ranked[0];
  const second = ranked[1];
  const confidence = second ? Math.max(0.5, Math.min(0.99, top.score / (top.score + second.score))) : 0.95;
  const multi_domain = ranked.filter((item) => item.score >= Math.max(4, top.score - 1)).map((item) => item.id);

  if (multi_domain.length > 1 && !project) {
    return { ok: true, capability: "multi_capability", status: "planned", confidence, candidates: ranked.slice(0, 4), required_capabilities: multi_domain, reason: "compound_request_requires_orchestration", production_deploy: false };
  }
...
export function capabilityRegistry() {
  return {
    version: "3.6",
    architecture: "multi_factory_capability_registry",
    capabilities: listCapabilities(),
    principles: {
      core_routes_work: true,
      domain_engines_remain_modular: true,
      unavailable_capabilities_are_never_faked: true,
      compound_requests_require_orchestration: true,
