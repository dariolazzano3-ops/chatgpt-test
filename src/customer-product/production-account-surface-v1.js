...
function accountShell() {
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AURENTARA SYSTEMS · Personal Business AI</title><style>body{font-family:system-ui,sans-serif;background:#f6f5f1;color:#191919;margin:0}.wrap{max-width:720px;margin:8vh auto;padding:24px}.card{background:#fff;border:1px solid #ddd9d0;border-radius:18px;padding:24px}h1{letter-spacing:-.04em}input,button{font:inherit;padding:11px;border-radius:10px;border:1px solid #d3d0c8;margin:5px 0;width:100%}button{background:#171717;color:#fff;cursor:pointer}.muted{color:#6e6a64;font-size:14px}.status{white-space:pre-wrap;margin-top:14px}</style></head><body><main class="wrap"><div class="card"><div class="muted">AURENTARA SYSTEMS · Personal Business AI</div><h1>Dein Business Workspace</h1><p class="muted">Production Identity und Tenant Isolation sind vorbereitet. AI-Verarbeitung echter Kundendaten bleibt bis zur separaten Freigabe deaktiviert.</p><input id="email" type="email" autocomplete="email" placeholder="E-Mail"><input id="password" type="password" autocomplete="current-password" placeholder="Passwort"><button id="signin">Anmelden</button><button id="signup">Account erstellen</button><button id="bootstrap">Workspace initialisieren</button><button id="signout">Abmelden</button><pre class="status" id="status"></pre></div></main><script>const s=document.getElementById('status');async function call(path,body){const r=await fetch('/customer/api/'+path,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body||{})});let j={};try{j=await r.json()}catch{}s.textContent=JSON.stringify(j,null,2);return j}const creds=()=>({email:document.getElementById('email').value,password:document.getElementById('password').value});signin.onclick=()=>call('account/signin',creds());signup.onclick=()=>call('account/signup',creds());bootstrap.onclick=()=>call('account/bootstrap',{business_name:'My Business'});signout.onclick=()=>call('account/signout',{});</script></body></html>`;
}
