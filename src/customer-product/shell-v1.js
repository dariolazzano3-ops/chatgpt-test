...
<section id="view-chat">
  ...
  <p class="muted">HAMYREN nutzt bestätigtes Business-Wissen, Ziele und Entscheidungen als Kontext. Kritische aktuelle Fragen werden ohne vertrauenswürdige Evidenz nicht geraten.</p>
  <div class="card">
    <div id="chatlog" class="chatlog"><div class="msg assistant">Willkommen bei HAMYREN. Starte die sichere Guest-Demo, um eine synthetische Business-Session anzulegen.</div></div>
...
async function send(){const box=document.getElementById('message'),message=box.value.trim();if(!message)return;await ensure();add('user',message);box.value='';const status=document.getElementById('chatstatus');status.textContent='Business Context wird geprüft …';try{const out=await api('chat',{method:'POST',body:JSON.stringify({message})});add('assistant',out.answer);status.textContent=out.risk_classification?.trusted_research_required?'Antwort mit Safety-Policy geprüft.':'Antwort mit Business Context geprüft.';}catch(e){if(e.body?.trusted_research_required)add('assistant','Für diese aktuelle oder sensible Frage brauche ich vertrauenswürdige aktuelle Quellen. Live Research ist in dieser sicheren V1 noch nicht aktiviert.');else add('assistant','Diese Aktion ist in der sicheren HAMYREN Customer Surface noch nicht verfügbar.');status.textContent=e.body?.error||e.message}}
