...
// Internal working commercial values only. This object is not customer-surface output and does not activate billing.
...
// Migration-safe compatibility adapter over the existing canonical Customer Economics plan IDs.
// It deliberately does not rewrite the legacy Founder/Standard payment lifecycle contract.
...
export function estimateBuildCreditsV1(input = {}) {
  ...
  const creditBoundaryRequiresAurentara = credits >= 8;
  const aboveBoundary = credits > 8;
  const aurentaraRequired = policyRequiresAurentara || hardReasons.length > 0 || creditBoundaryRequiresAurentara || aboveBoundary;
...
export function customerSafeBuildCreditPresentationV1(estimate = {}) {
  if (estimate.aurentara_required === true) {
    return { route: 'AURENTARA_PROFESSIONAL', message: 'This project needs AURENTARA professional implementation.', build_credits_required: null, review_required: true };
  }
  ...
  return {
    ...
    message: estimate.review_required ? `This build requires ${credits} Build Credits and a review before execution.` : `This build requires ${credits} Build Credits.`,
...
export function classifyHamyrenFairUseV1(input = {}) {
  ...
  const machineSpeed = input.machine_speed === true;
...
function emptyWallet(ctx) {
  return {
    schema: 'hamyren.build-credit-wallet.v1', tenant_id: ctx.tenant_id, business_id: ctx.business_id,
    balances: { INCLUDED: emptySource(), PURCHASED: emptySource() }, grants: {}, reservations: {}, operation_index: {}, entries: [], created_at: now(), updated_at: now()
  };
}
...
export function createBuildCreditEntitlementExtension(options = {}) {
  ...
  async function ensureBuildCreditWallet(ctx = {}) {
    ...
    const wallet = emptyWallet(auth);
    const written = await store.put(scope(auth.tenant_id), 'build-credit-wallets', auth.business_id, wallet, { expected_revision: 0 });
  ...
  async function grantMonthlyIncludedBuildCredit(ctx = {}, input = {}) {
    ...
    const period = clean(input.period, 32), subscriptionId = clean(input.subscription_id, 160);
    ...
    if (!['synthetic_test', 'payment_lifecycle_projection'].includes(clean(input.source, 80))) return { ok: false, error: 'BUILD_CREDIT_GRANT_SOURCE_NOT_ALLOWED' };
  ...
  async function reserveBuildCredits(ctx = {}, input = {}) {
    ...
    if (!operationId || !estimate?.schema_version) return { ok: false, error: 'BUILD_CREDIT_PREFLIGHT_AND_OPERATION_REQUIRED' };
    ...
    return mutate(auth, (wallet) => {
      const priorReservationId = wallet.operation_index[operationId];
      if (priorReservationId && wallet.reservations[priorReservationId]) return { duplicate: true, reservation: clone(wallet.reservations[priorReservationId]) };
      ...
      for (const source of ['INCLUDED', 'PURCHASED']) {
        ...
        if (take > 0) {
          wallet.balances[source].available -= take; wallet.balances[source].reserved += take; allocations.push({ source, credits: take }); remaining -= take;
        }
      }
      const reservationId = clean(input.reservation_id, 200) || `build:${auth.business_id}:${operationId}`;
      const reservation = {
        reservation_id: reservationId, operation_id: operationId, mission_id: clean(input.mission_id, 180) || null,
        credits: amount, allocations, status: 'RESERVED', estimate: clone(estimate), retry_count: 0,
        execution_authorized: false, production_deploy: false, created_at: now(), settled_at: null
      };
  ...
  async function settleBuildCredits(ctx = {}, input = {}) {
    ...
    return mutate(auth, (wallet) => {
      ...
      const consumedBySource = [], restoredBySource = [];
      for (const allocation of reservation.allocations) {
        const source = allocation.source, allocated = Number(allocation.credits || 0);
        const consumed = Math.min(allocated, remainingConsume), restored = allocated - consumed;
  ...
  async function buildCreditSnapshot(ctx = {}) {
    ...
    return {
      ok: true, tenant_id: auth.tenant_id, business_id: auth.business_id,
      credits: { available: total(wallet, 'available'), reserved: total(wallet, 'reserved'), consumed: total(wallet, 'consumed'), restored: total(wallet, 'restored'), sources: clone(wallet.balances) },
      reservations: Object.values(wallet.reservations).map(clone), grants: Object.values(wallet.grants).map(clone), entries: clone(wallet.entries)
    };
  }
