...
import { evaluatePremiumWebsiteStandard, createPremiumCustomerDeliverySummary } from './premium-standard-v1.js';
import { createAssetMediaPipeline } from './asset-media-pipeline-v1.js';
import { createEvidenceSafeContentContract, runContentRenderGuard, createSeoEvidenceBundle, applySeoStaticArtifacts } from './content-seo-evidence-v2.js';
...
export function runWebOperatingSystemV2(input = {}, options = {}) {
  ...
  const build=buildAutonomousPremiumWebsite({mission,quality_level:quality,use_framer_visual_specialist:useFramer,routing_context:{native_premium:!useFramer},screenshot_report:input.screenshot_report,framer_status:input.framer_status||{},approved_reference_id:input.approved_reference_id,approved_reference_hash:input.approved_reference_hash,reference_registry_entry:input.reference_registry_entry},{...options,max_visual_repair_attempts:qualityPolicy.repair_attempts});
  ...
  const contentEvidence=createEvidenceSafeContentContract({mission,project_scope:mission.project_scope_key||mission.project_slug,claims:input.content_claims||input.claims||[],local_business_data:input.local_business_data||{},content:input.content||mission.existing_content||{},attempted_render_claim_ids:input.attempted_render_claim_ids||[]});
  const seoEvidence=createSeoEvidenceBundle({mission,content_contract:contentEvidence,seo,architecture,page_intents:pageIntents.pages,environment:'staging',nap_occurrences:input.nap_occurrences||[],redirects:input.redirects||[],content:input.content||mission.existing_content||{}});
  const seoStaticArtifacts=applySeoStaticArtifacts(build,seoEvidence,{base_url:mission.existing_domain||null});
  const contentRenderGuard=runContentRenderGuard(build.artifact,contentEvidence);
  const localSeo=seoEvidence.local_seo||createLocalSeoContract(mission,input.local_business_data||{});const programmaticSeo=createProgrammaticSeoContract(input.programmatic_seo||{});const localization=createLocalizationV2(mission,input.localization||mission.localization,pageIntents.pages);
  ...
  const motion=createMotionDesignContract(mission.motion_intent||[],{quality_level:quality});const motionRuntime=createMotionRuntimePlan(motion);const motionQuality=createMotionQualityGate(motion);const interactions=createAdvancedInteractionContract(input.interactions||[]);
  const assetInventory=createAssetInventory(input.assets||mission.existing_brand?.assets||[]);const imageContracts=arr(input.images).map(createImageOptimizationContract);const mediaPipeline=createAssetMediaPipeline({project_scope:mission.project_scope_key||mission.project_slug,assets:input.assets||mission.existing_brand?.assets||[],images:input.images||[],videos:input.videos||[]});const migration=createMigrationIntelligence(mission.existing_website||input.existing_website||{});
  ...
  const premiumEvidence=input.premium_evidence||{};
  const premiumStandard=evaluatePremiumWebsiteStandard({...premiumEvidence,project_ref:{project_id:mission.project_slug,scope_key:input.project_context?.project?.scope_key||null},industry:mission.industry,existing_web_quality_score:qualityScore,
    input_readiness:premiumEvidence.input_readiness||{values:{business_identity:mission.business_name,business_model:mission.industry,products_services:mission.services,target_customers:mission.target_audience,primary_conversion:mission.conversion_goal}},
    brand_readiness:premiumEvidence.brand_readiness||{path:mission.existing_brand?'USE_EXISTING_BRAND':'LIGHT_REFINE',branding_complete:true,issues:mission.existing_brand?[]:[{severity:'WARN',code:'BRAND_REFINEMENT_REVIEW_REQUIRED'}]},
    assets:premiumEvidence.assets||input.assets||[],preview_qa:premiumEvidence.preview_qa||selfHeal.status,responsive_qa:premiumEvidence.responsive_qa||selfHeal.final_QA.responsive,
    seo_evidence:premiumEvidence.seo_evidence||selfHeal.final_QA.seo,local_seo_evidence:premiumEvidence.local_seo_evidence||localSeo,performance_evidence:premiumEvidence.performance_evidence||{prelaunch_lab:selfHeal.final_QA.performance},
    accessibility_evidence:premiumEvidence.accessibility_evidence||{automated:selfHeal.final_QA.accessibility},privacy_evidence:premiumEvidence.privacy_evidence||security.consent,security_evidence:premiumEvidence.security_evidence||security,project_isolation:isolation,evaluated_at:options.now});
  const premiumCustomerDelivery=createPremiumCustomerDeliverySummary({what_was_built:['website'],business_goals_addressed:[strategy.primary_goal],pages:architecture.site_map.map((page)=>page.page_id),major_features:composition.component_specs.map((item)=>item.component),quality_checks:['existing web quality score','premium evidence aggregation'],premium_score:premiumStandard.weighted_score,
    remaining_limitations:[...(premiumStandard.not_verified_hard_gates.length?['mandatory premium hard-gate evidence remains NOT_VERIFIED']:[]),...(premiumStandard.human_review.state!=='APPROVED_FOR_PREMIUM_DELIVERY'?['final human quality gate not approved for premium delivery']:[])],
    launch_state:premiumStandard.launch_readiness.public_launch_ready?'PUBLIC LAUNCH READY':'NOT READY',ownership_accounts:[],next_actions:premiumStandard.delivery_readiness.premium_delivery_ready?[]:['complete missing premium evidence and human review'],care_options:['PROJECT_INCLUDED','OPTIONAL','ONGOING_CARE']});
  const blocking=[tokenValidation.status==='BLOCK',content.brand_voice.status==='BLOCK',content.consistency.status==='BLOCK',composition.component_quality.some((q)=>q.status==='BLOCK'),selfHeal.status==='BLOCK',motionQuality.status==='BLOCK',security.status==='BLOCK',isolation.status==='BLOCK',mediaPipeline.validation.status==='BLOCK',contentEvidence.status==='BLOCK',contentRenderGuard.status==='BLOCK',seoEvidence.status==='BLOCK',seoStaticArtifacts.ok!==true,qualityScore.status==='FAIL',build.ok!==true].some(Boolean);
  ...
  const delivery={
    ...
    design_intent:build.design_intent,design_system:designSystem,content_version:websiteVersion.content_version,content_planning:content.plans,content_evidence:{contract:contentEvidence,render_guard:contentRenderGuard},visual_references:build.visual_reference_fusion,competitor_insights:competitor,differentiation,components:composition.component_specs,motion:{contract:motion,runtime:motionRuntime,quality:motionQuality},localization,
    SEO:{architecture:selfHeal.models.seo,QA:selfHeal.final_QA.seo,evidence:seoEvidence,local:localSeo,programmatic:programmaticSeo},accessibility:selfHeal.final_QA.accessibility,performance:selfHeal.final_QA.performance,CRO:build.CRO_result,trust,visual_quality:build.visual_quality,visual_fidelity:build.visual_fidelity,
    repair_history:{general:selfHeal.repair_history,legacy:build.repair_history},business_integration_hooks:integration,AI_content_request:content.AI_request,analytics_events:events,provider_route:route,build_profile:build.artifact.build_profile||null,cms:{contract:cmsContract,assessment:cmsAssessment},deployment_artifact:deployment,
    rights_status:{originality,asset_inventory:assetInventory,media_pipeline:mediaPipeline},security,forms:[form],advanced_interactions:interactions,migration,versioning:{website:websiteVersion,build:buildVersion},change_impact:impact,blast_radius:blastRadius,regression,rollback,experiment_architecture:{generic:experiment,posthog:posthogExperiment},learning_contracts:learning,
    quality_score:qualityScore,premium_standard:premiumStandard,premium_customer_delivery:premiumCustomerDelivery,operator_overrides:{values:clone(input.operator_overrides||{}),audited:true},cost_metadata:route.cost_governance||createCostGovernance(route,{synthetic_test_data_only:mission.synthetic_test_data_only}),warnings,production_status:'DISABLED',production_deploy:false
  };
  const root=build.artifact.project_root;const files=build.artifact.files;files[`${root}/web-os-v2-strategy.json`]=JSON.stringify(strategy,null,2);files[`${root}/web-os-v2-architecture.json`]=JSON.stringify(architecture,null,2);files[`${root}/web-os-v2-page-intents.json`]=JSON.stringify(pageIntents,null,2);files[`${root}/web-os-v2-content-plan.json`]=JSON.stringify(content,null,2);files[`${root}/web-os-v2-content-evidence.json`]=JSON.stringify({contract:contentEvidence,render_guard:contentRenderGuard},null,2);files[`${root}/web-os-v2-seo-evidence.json`]=JSON.stringify(seoEvidence,null,2);files[`${root}/web-os-v2-self-healing.json`]=JSON.stringify(selfHeal,null,2);files[`${root}/web-os-v2-motion-contract.json`]=JSON.stringify({contract:motion,runtime:motionRuntime,quality:motionQuality},null,2);files[`${root}/web-os-v2-media-manifest.json`]=JSON.stringify(mediaPipeline,null,2);files[`${root}/web-os-v2-delivery-manifest.json`]=JSON.stringify(delivery,null,2);files[`${root}/web-os-v2-observability.json`]=JSON.stringify(obs,null,2);files[`${root}/premium-standard-v1.json`]=JSON.stringify(premiumStandard,null,2);files[`${root}/premium-customer-delivery-v1.json`]=JSON.stringify(premiumCustomerDelivery,null,2);
  return {ok:!blocking,status,compiler:compiled,mission,strategy,information_architecture:architecture,user_journeys:journeys,page_intents:pageIntents,recipe,brand_direction:brand,design_system:designSystem,design_token_validation:tokenValidation,typography,layout,content_system:content,composition,trust,form,competitor_intelligence:competitor,differentiation,originality,cms:{contract:cmsContract,assessment:cmsAssessment},provider_route:route,build_profile:build.build_profile||build.artifact.build_profile||null,premium_build:build,content_evidence:delivery.content_evidence,SEO:delivery.SEO,localization,responsive:selfHeal.final_QA.responsive,accessibility:selfHeal.final_QA.accessibility,performance:selfHeal.final_QA.performance,self_healing:selfHeal,motion:delivery.motion,interactions,assets:{inventory:assetInventory,images:imageContracts,media:mediaPipeline},migration,integrations:integration,security,versioning:delivery.versioning,change_impact:impact,blast_radius:blastRadius,regression,rollback,deployment,experiments:delivery.experiment_architecture,quality_score:qualityScore,premium_standard:premiumStandard,premium_customer_delivery:premiumCustomerDelivery,delivery_manifest:delivery,observability:obs,artifact:build.artifact,correlation_id:correlation,project_isolation:isolation,variable_cost_eur:0,production_deploy:false};
}
