...
export function evaluateVisualFidelity(reference = {}, implementation = {}, options = {}) {
  ...
  for (const key of ['background', 'surface', 'text', 'muted', 'accent', 'border']) {
    addCheck(checks, 'colors', `colors.${key}`, reference.colors?.[key], implementation.colors?.[key], 2);
  }
  ...
  for (const page of referencePages) {
    addCheck(
      checks,
      'section_order',
      `pages.${page.id}.section_order`,
      page.section_order,
      implementationPages.get(String(page.id))?.section_order,
      3
    );
  }
  ...
  for (const [name, component] of referenceComponents.entries()) {
    addCheck(
      checks,
      'component_geometry',
      `components.${name}.geometry`,
      component.geometry,
      implementationComponents.get(name)?.geometry,
      2
    );
  }
  ...
  addCheck(
    ...
    'interactions.native_reproducible',
    reference.interactions?.coverage?.native_reproducible,
    implementation.interactions?.coverage?.implemented_native,
    3
  );
  ...
  const screenshot = options.screenshot_report || {
    status: 'NOT_EXECUTED_RUNTIME_UNAVAILABLE',
    executed: false,
    pixel_comparison_claimed: false
  };

  const expectedCategories = new Set(['layout_similarity', 'spacing', 'typography', 'colors', 'component_geometry', 'responsive_behavior', 'section_order', 'interaction_coverage']);
  ...
  return {
    ...
    blocking_differences: [
      ...blocking,
      ...failed.map(({ category, path, expected, actual }) => ({ category, path, expected, actual }))
    ],
    warnings: screenshot.executed === true
      ? []
      : [{ code: 'PIXEL_COMPARISON_NOT_EXECUTED', message: 'No pixel-level claim is included in the structured fidelity score.' }],
