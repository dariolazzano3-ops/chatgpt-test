...
(() => {
  ...
  function renderReferenceHq() {
    ...
    root.querySelector('[data-rf-portfolio-open]')?.addEventListener('click', () => { if (typeof go==='function') go('projects'); });root.querySelector('[data-rf-new-project]')?.addEventListener('click', () => { if(typeof go==='function')go('projects');setTimeout(()=>document.getElementById('pm-new-project')?.click(),0); });root.querySelector('#rf-project-search')?.addEventListener('input',e=>{state.referenceHqProjectQuery=e.target.value;renderReferenceHq();document.getElementById('rf-project-search')?.focus()});
