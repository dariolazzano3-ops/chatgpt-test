...
export function executeNaturalEditPlan({ css = "", html = "", plan }) {
  ...
  for (const op of Array.isArray(plan.operations) ? plan.operations : []) {
    ...
    if (target === ".rocket" && action === "scale") {
      ...
      cssOverrides.push(overrideBlock(".rocket", {
        filter: "drop-shadow(0 18px 24px rgba(0,0,0,.42)) drop-shadow(0 0 22px rgba(180,215,255,.16))",
        "backface-visibility": "hidden"
      }));
      cssOverrides.push(overrideBlock(".rocket-body, .booster", {
        "box-shadow": "inset -10px 0 18px rgba(0,0,0,.28), inset 8px 0 16px rgba(255,255,255,.12)"
      }));
      applied.push({ target, action, value: "high" });
    } else if (target === ".rocket" && action === "lighting" && op.value === "cinematic") {
      cssOverrides.push(overrideBlock(".rocket-body, .booster", {
        background: "linear-gradient(90deg, #8e99a6 0%, #eef4f8 28%, #aab6c2 58%, #55606c 100%)"
      }));
      ...
      cssOverrides.push(overrideBlock(".smoke", {
        opacity: Math.min(1, 0.48 * density).toFixed(2),
        transform: `scale(${Math.min(2.4, 0.9 + density * 0.35).toFixed(2)})`
      }));
      ...
      cssOverrides.push(overrideBlock(".site-header", {
        background: `rgba(6, 9, 18, ${opacity})`,
