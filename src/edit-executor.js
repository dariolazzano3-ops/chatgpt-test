...
export function executeNaturalEditPlan({ css = "", html = "", plan }) {
  ...
  for (const op of Array.isArray(plan.operations) ? plan.operations : []) {
    ...
    const target = semantic && !["theme", "headline"].includes(semantic) ? resolveSemanticSelector(analysis, semantic, fallbackTarget) : fallbackTarget;
    const action = clean(op.action, 120);
    if (!target) continue;
    if (semantic === "headline" && action === "replace_text") {
      const value = clean(op.value, 180);
      if (!value || !/<h1\b[^>]*>[\s\S]*?<\/h1>/i.test(nextHtml)) continue;
      nextHtml = nextHtml.replace(/(<h1\b[^>]*>)[\s\S]*?(<\/h1>)/i, `$1${escapeHtml(value)}$2`);
      applied.push({ target: "h1", action, value, semantic });
  ...
  const changedFiles = [];
  if (nextHtml !== html) changedFiles.push("index.html");
  if (nextCss !== css) changedFiles.push("styles.css");
  return { ok: true, css: nextCss, html: nextHtml, applied, analysis, override_management: { mode: "replace", legacy_overrides_removed: css.includes(LEGACY_MARKER), managed_block: Boolean(managedBlock) }, changed_files: changedFiles };
}
