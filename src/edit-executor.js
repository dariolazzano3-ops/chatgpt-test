...
function replaceSubheadline(html, value) { const heroLead = /(<(?:p|span)\b[^>]*class=["'][^"']*(?:hero-text|hero-lead|lead|subtitle|subheadline)[^"']*["'][^>]*>)[\s\S]*?(<\/(?:p|span)>)/i; if (heroLead.test(html)) return html.replace(heroLead, `$1${escapeHtml(value)}$2`); const afterH1 = /(<h1\b[^>]*>[\s\S]*?<\/h1>[\s\S]*?<p\b[^>]*>)[\s\S]*?(<\/p>)/i; return afterH1.test(html) ? html.replace(afterH1, `$1${escapeHtml(value)}$2`) : html; }
