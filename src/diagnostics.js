...
function normalizeBaseUrl(value) {
  const candidate = clean(value || "https://de.sentry.io", 200);
...
function sentryHeaders(token) {
  return {
    ...
    "user-agent": "riosystems-sentry-diagnostics/1.0"
  };
}
...
export async function handleDiagnostics(request, env = {}) {
  ...
  if (
    url.pathname !== "/factory/diagnostics" &&
    url.pathname !== "/factory/diagnostics/github" &&
    url.pathname !== "/factory/diagnostics/sentry"
  ) return null;

  if (url.pathname === "/factory/diagnostics/sentry") {
    return sentryDiagnostics(request, env);
  }
