...
export function validateExternalAction(action = {}, policy = {}) {
  ...
  return { ok: errors.length === 0, errors, action: errors.length ? null : { type, url: url.href, method, headers, body }, limits: { max_body_bytes: maxBodyBytes, timeout_ms: Math.min(Math.max(Number(policy.timeout_ms) || 5000, 100), 10000) }, supervised: true, automatic_execution: false, production_deploy: false };
}
...
export async function executeExternalAction(action = {}, policy = {}, transport) {
  ...
  try {
    ...
    return { ok: statusCode >= 200 && statusCode < 400, status: statusCode >= 200 && statusCode < 400 ? 'COMPLETED' : 'FAILED', status_code: statusCode || null, duration_ms: Math.max(0, Date.now() - started), external_side_effect: true, supervised: true, automatic_execution: false, production_deploy: false };
  } catch (error) {
    return { ok: false, status: 'FAILED', error: clean(error?.message || error, 300), duration_ms: Math.max(0, Date.now() - started), external_side_effect: true, supervised: true, automatic_execution: false, production_deploy: false };
  }
}

export function externalActionManifest() {
  return { version: '4.3', mode: 'supervised', enabled_types: [...SUPPORTED], blocked_types: [...BLOCKED], https_only: true, allowlist_required: true, inline_secrets_allowed: false, automatic_execution: false, production_deploy: false };
}
