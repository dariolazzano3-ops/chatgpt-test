...
const OPTIONAL_PROPERTIES = Object.freeze([
  'make_execution_id',
  'outcome',
  'failure_code'
]);
...
export function validatePostHogStagingEvent(event = {}) {
  ...
  if (event.distinct_id !== DISTINCT_ID) return false;
  ...
  if (serialized.includes('@') || serialized.includes('http://') || serialized.includes('https://')) return false;
  return true;
}
