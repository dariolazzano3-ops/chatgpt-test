...
function decorateKnowledge(root,p){const panel=root.querySelector('[data-knowledge-review-panel]');const cards=root.querySelector('[data-source-cards]');if(!panel||!cards)return;if(panel.previousElementSibling!==cards)cards.insertAdjacentElement('afterend',panel);const view=p?.workspace?.knowledge_review||{};const prepare=panel.querySelector('[data-knowledge-prepare]');if(prepare&&!prepare.disabled){const desired=view.status==='IN_REVIEW'||view.status==='CHANGES_PENDING'?'Änderungen mit KI neu aufbereiten':'Mit KI aufbereiten';if(prepare.textContent!==desired)prepare.textContent=desired}const approve=panel.querySelector('[data-knowledge-approve]');if(approve&&!approve.disabled&&approve.textContent!=='Für Ferrari freigeben')approve.textContent='Für Ferrari freigeben';let meta=panel.querySelector('[data-v2-review-meta]');if(!meta){meta=document.createElement('div');meta.dataset.v2ReviewMeta='true';meta.className='source-v2-review-meta';panel.appendChild(meta)}let desiredClass='source-v2-review-meta',desiredText='';if(Number(view.conflict_count||0)>0){desiredClass='source-v2-review-meta conflict';desiredText='⚠️ '+view.conflict_count+' widersprüchliche Angabe(n). Freigabe bleibt gesperrt, bis der Konflikt human aufgelöst wurde.'}else if(view.status==='APPROVED'){desiredText='✅ Freigegeben'+(view.approved_at?' · '+new Date(view.approved_at).toLocaleString('de-DE'):'')+(view.approved_by?' · '+view.approved_by:'')+(view.approved_knowledge_revision?' · Knowledge Revision '+view.approved_knowledge_revision:'')}else if(view.status==='CHANGES_PENDING'){desiredText='Neue Änderungen vorhanden. Bitte Projektakte erneut aufbereiten und prüfen. Factories bleiben bis zur erneuten Freigabe gesperrt.'}else if((view.source_count||0)===0){desiredText='Noch kein Rohmaterial. KI-Aufbereitung und finale Freigabe sind noch nicht abgeschlossen.'}else{desiredText='Rohmaterial vorhanden. Der nächste Schritt ist die KI-Aufbereitung, danach die menschliche Prüfung.'}if(meta.className!==desiredClass)meta.className=desiredClass;if(meta.textContent!==desiredText)meta.textContent=desiredText}
