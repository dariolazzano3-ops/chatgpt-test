...
.source-purpose-toggles{display:flex;gap:7px;flex-wrap:wrap;margin-top:7px}
.source-purpose-toggle{border:1px solid var(--line);background:#fff;border-radius:10px;padding:8px 10px;font-size:12px;font-weight:700;cursor:pointer}
.source-purpose-toggle.active{background:#111;color:#fff;border-color:#111}
.source-purpose-toggle[disabled]{opacity:.55;cursor:wait}
.source-purpose-help{font-size:11px;color:var(--muted);margin-top:6px;line-height:1.4}
...
const PURPOSE_LABEL={INFORMATION_EXTRACTION:'Informationen auslesen',VISUAL_USAGE:'Visuell verwenden',BOTH:'Beides'};
const purposeFlags=p=>({visual:['VISUAL_USAGE','BOTH'].includes(p),info:['INFORMATION_EXTRACTION','BOTH'].includes(p)});
const purposeFromFlags=f=>f.visual&&f.info?'BOTH':f.info?'INFORMATION_EXTRACTION':'VISUAL_USAGE';
const togglePurpose=(purpose,kind)=>{const f=purposeFlags(purpose);if(kind==='visual')f.visual=!f.visual;else f.info=!f.info;if(!f.visual&&!f.info){if(kind==='visual')f.visual=true;else f.info=true}return purposeFromFlags(f)};
...
function activate(root){if(!root||root.dataset.intakeUxV2==='true')return;root.dataset.intakeUxV2='true';const h2=root.querySelector('h2');if(h2)h2.textContent='Wäschekorb / Rohmaterial';const eyebrow=root.querySelector('.eyebrow');if(eyebrow)eyebrow.textContent='PROJECT FERRARI · INTELLIGENT PROJECT INTAKE';makeComposer(root);const scope=root.dataset.scope||root.__sourcePayload?.identity?.scope_key;if(scope)refresh(root,scope).catch(error=>{if(typeof window.setError==='function')window.setError(error);setMessage(root,'Rohmaterial konnte nicht geladen werden: '+String(error?.data?.error||error?.message||'UNKNOWN'),'error')})}
function tick(){document.querySelectorAll('[data-project-source-intake]').forEach(root=>{activate(root);const p=root.__sourcePayload;if(p){const cards=root.querySelector('[data-source-cards]');if(cards&&!cards.querySelector('[data-v2-source-card],[data-v2-source-empty]'))renderCards(root,p);decorateKnowledge(root,p)}})}
