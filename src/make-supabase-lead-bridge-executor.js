...
export async function runMakeSupabaseLeadBridgeOnce(plan = {}, approvals = {}, runtime = {}) {
  ...
  let makeResult;
  try { makeResult = await runtime.run_make({ scope: clone(plan.scope), input: clone(plan.bridge_contract.input), scenario_id: plan.providers.automation.verified_scenario_id }); }
  catch (error) { return { ok: false, error: 'MAKE_SUPABASE_BRIDGE_MAKE_FAILED', message: clean(error?.message), supabase_write_performed: false, production_deploy: false }; }
  if (!validMakeResult(makeResult, plan)) return { ok: false, error: 'MAKE_SUPABASE_BRIDGE_MAKE_RESULT_REJECTED', supabase_write_performed: false, make_result: clone(makeResult), production_deploy: false };
  ...
  while (attempts < 2) {
    ...
    } catch (error) { lastError = clean(error?.message || 'SUPABASE_PERSISTENCE_FAILED'); }
  }
  ...
  if (!validPersistence(persist)) {
    return {
      ...
      attempts,
      dead_letter: { required: true, scope_key: plan.scope.scope_key, idempotency_key: plan.bridge_contract.input.lead.idempotency_key, make_execution_id: request.make.execution_id, reason: lastError || 'PERSISTENCE_NOT_VERIFIED' },
