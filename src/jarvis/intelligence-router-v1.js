...
function astraPostReviewSystemPrompt() {
  return [
    ...
    'This does not waive semantic correctness: the verified code delta must still satisfy the owner\'s substantive implementation goal.',
    'For implementation jobs, trusted local commit/publication and private acceptance checks run only AFTER your PASS. An unchanged HEAD or a verified uncommitted diff is therefore expected and MUST NOT by itself cause REPAIR or BLOCK.',
    'Do not require post-review delivery/reporting fields such as final commit id, final private-deploy test evidence, final job cost, or next-blocker summary as pre-PASS implementation criteria. Trusted finalization supplies those after PASS and fails closed if its checks fail.',
    'The original owner goal is immutable and authoritative.',
    'Decide whether the verified implementation semantically satisfies that goal without broadening scope. REPAIR only for an actual code/scope defect, missing required repository change, or contradictory verification evidence.',
    'Return JSON only with exactly these keys:',
