...
function normalizeRow(row, operatorId = null) {
  if (!row || typeof row !== 'object' || !validRuntime(row.runtime)) {
    throw new Error('OPERATOR_RUNTIME_STORE_INVALID_ROW');
  }
  if (operatorId && row.runtime.operator_id !== operatorId) throw new Error('OPERATOR_RUNTIME_STORE_SCOPE_MISMATCH');
  if (!Number.isInteger(Number(row.revision)) || Number(row.revision) !== row.runtime.revision) {
    throw new Error('OPERATOR_RUNTIME_STORE_REVISION_MISMATCH');
  }
  return clone(row.runtime);
}

function conflictResult(expectedRevision, actualRevision = null) {
  return {
    ok: false,
    error: 'STORE_REVISION_CONFLICT',
    expected_revision: expectedRevision,
    actual_revision: Number.isInteger(Number(actualRevision)) ? Number(actualRevision) : null
  };
}

export function createSupabaseOperatorRuntimeStore({
  supabase_url,
  service_role_key,
  table_name = 'riosystems_operator_runtime_v1',
  ...
  async function request(query, init = {}) {
    let response;
    try {
      response = await fetch_impl(runtimeUrl(supabaseUrl, tableName, query), {
        ...init,
  ...
  return {
    ...
    async load(operatorId) {
      ...
      const query = `operator_id=eq.${encodeURIComponent(key)}&select=operator_id,revision,runtime&limit=1`;
      const response = await request(query, { method: 'GET', headers: { accept: 'application/json' } });
    ...
    async compareAndSwap(runtime, expectedRevision) {
      ...
      if (runtime.revision !== expected + 1) {
        return {
          ok: false,
          error: 'STORE_NEXT_REVISION_INVALID',
          expected_next_revision: expected + 1,
          actual_next_revision: runtime.revision
        };
      }
      const at = clean(clock(), 80) || new Date().toISOString();
      const query = `operator_id=eq.${encodeURIComponent(runtime.operator_id)}&revision=eq.${expected}&select=operator_id,revision,runtime`;
      const response = await request(query, {
        method: 'PATCH',
        headers: { prefer: 'return=representation', accept: 'application/json' },
        body: JSON.stringify({ revision: runtime.revision, runtime: clone(runtime), updated_at: at })
      });
      ...
      const current = await this.load(runtime.operator_id);
      if (!current) return { ok: false, error: 'OPERATOR_RUNTIME_NOT_FOUND' };
      return conflictResult(expected, current.revision);
    }
  };
}
