...
function approvalFor(task, approvals = {}) {
  const engine = executionEngine(task);
  return approvals[task.task_id] || approvals[engine] || approvals.default || {};
}
...
function attachWebDispatch(mission, taskId, dispatch = {}, request) {
  ...
  task.inputs = {
    ...
    factory_request_ref: dispatch.request_ref || null,
    web_dispatch_supervised: true,
...
export async function superviseMission(missionInput, approvals = {}, options = {}) {
  let mission = clone(missionInput);
  const events = [];
  const maxRounds = Math.max(1, Math.min(Number(options.max_rounds) || 20, 100));
  const persist = typeof options.persist === 'function' ? options.persist : async () => {};
  const dispatchWeb = typeof options.dispatch_web === 'function' ? options.dispatch_web : null;
  const observeWeb = typeof options.observe_web === 'function' ? options.observe_web : null;

  for (let round = 1; round <= maxRounds; round += 1) {
    if (terminalMission(mission.status)) break;
    ...
    for (const task of mission.tasks.filter((item) => item.state === 'RUNNING' && executionEngine(item) === 'web')) {
      ...
      events.push({ round, task_id: task.task_id, engine: 'web', action: 'reconcile', status: job.status });
      await persist(clone(mission), { reason: 'web_reconciled', task_id: task.task_id, round });
      progressed = true;
    }

    const ready = mission.tasks.filter((task) => task.state === 'READY');
    for (const taskSnapshot of ready) {
      const task = mission.tasks.find((item) => item.task_id === taskSnapshot.task_id);
      if (!task || task.state !== 'READY') continue;
      const engine = executionEngine(task);
      const approval = approvalFor(task, approvals);
      const result = await executeMissionTask(mission, task.task_id, approval, taskOptions(task, options));
      if (!result.ok) {
        events.push({ round, task_id: task.task_id, engine, action: 'blocked', error: result.error });
        continue;
      }
      ...
      if (engine === 'web') {
        ...
        events.push({ round, task_id: task.task_id, engine, action: 'execute', state: mission.tasks.find((item) => item.task_id === task.task_id)?.state || null });
      }
      await persist(clone(mission), { reason: engine === 'web' ? 'web_dispatched' : 'task_executed', task_id: task.task_id, round });
    }

    if (!progressed) break;
  }

  const pendingWebTasks = mission.tasks.filter((task) => task.state === 'RUNNING' && executionEngine(task) === 'web').map((task) => task.task_id);
  const blockedReadyTasks = mission.tasks.filter((task) => task.state === 'READY').map((task) => task.task_id);
  return {
    ok: true,
    mission,
    ...
    pending_web_tasks: pendingWebTasks,
    ready_but_not_executed: blockedReadyTasks,
    events,
    production_deploy: false,
...
export function missionSupervisorManifest() {
  return {
    version: '4.9',
    mode: 'supervised_multi_factory_loop',
    supported_engines: ['web', 'automation', 'ai', 'business'],
    durable_persistence_hook: true,
    web_dispatch_hook: true,
    web_observation_hook: true,
    explicit_adapter_approval_required: true,
    ai_injected_runner_required: true,
    automatic_multi_factory_execution: false,
    production_deploy: false
  };
}
