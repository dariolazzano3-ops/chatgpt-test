...
assert.ok(runtimeMajor > 4 || (runtimeMajor === 4 && runtimeMinor >= 9), `runtime factory_version ${runtime.factory_version} must be >= 4.9`);
assert.equal(runtime.production_deploy, false);
for (const capability of ['supervised_multi_factory_mission_loop','persisted_multi_factory_mission_supervisor','mission_supervisor_persistence_hook','mission_supervisor_web_dispatch_hook','mission_supervisor_web_observation_hook','mission_supervisor_resume_safe','mission_supervisor_workflow']) assert.equal(runtime.capabilities[capability], true);
...
for (const file of ['src/mission-supervisor.js','scripts/mission-supervisor.mjs','scripts/mission-supervisor-smoke.mjs','.github/workflows/mission-supervisor.yml']) assert.equal(fs.existsSync(file), true, `${file} missing`);
