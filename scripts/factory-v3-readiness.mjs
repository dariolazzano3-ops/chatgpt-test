...
const jobState = read('scripts/factory-job-state.mjs');
const qaRepairLoop = read('scripts/qa-repair-loop.mjs');
...
requireText('Factory Control runs bounded Visual QA repair loop', control, 'bounded QA repair loop');
requireText('Factory Control persists durable jobs', control, 'Initialize durable Factory job');
requireText('Factory Control caps automatic QA attempts', control, 'MAX_QA_ATTEMPTS: 3');
...
requireText('Durable job state is stored under factory-state/jobs', jobState, 'factory-state/jobs/${id}.json');
requireText('Durable job state hard-disables production deploy', jobState, 'production_deploy: false');
requireText('QA repair loop exposes READY_FOR_REVIEW', qaRepairLoop, "status: 'READY_FOR_REVIEW'");
requireText('QA repair loop exposes FAILED', qaRepairLoop, "status: 'FAILED'");
requireText('QA repair loop has maximum three attempts', qaRepairLoop, 'Math.min(3');
requireText('QA repair loop only auto-fixes classified overflow', qaRepairLoop, 'horizontal overflow|scroll overflow');
requireText('QA repair loop prevents repeated failure loops', qaRepairLoop, 'classified.signature === previousFailureSignature');
requireText('QA repair loop never enables production', qaRepairLoop, 'production_deploy: false');
