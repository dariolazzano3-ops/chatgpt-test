...
const inactiveIdentity = createCustomerIdentityAdapter();
assert.equal((await inactiveIdentity.resolve({ tenant_id: 'tenant-a', assertion: {} })).error, 'CUSTOMER_IDENTITY_PROVIDER_NOT_CONFIGURED');
...
const store = createDurableCustomerStoreAdapter({ driver });
...
const inactiveRate = createDistributedRateLimitAdapter();
assert.equal((await inactiveRate.check({ key: 'x', route_class: 'chat' })).limited, true);
...
assert.equal((await rate.check({ key: 'tenant-a:chat', route_class: 'chat', limit: 2 })).limited, false);
assert.equal((await rate.check({ key: 'tenant-a:chat', route_class: 'chat', limit: 2 })).limited, true);

const deletion = createCustomerDeletionExecutor({ store });
...
const observed = [];
const observability = createCustomerObservabilityAdapter({ sink_active: false, emit: async (event) => { observed.push(event); return { ok: true }; } });
const obs = await observability.record({
  ...
  attributes: { latency_ms: 42, message: 'private message', email: 'user@example.test', nested: { token: 'secret-token', status: 'ok' } }
});
