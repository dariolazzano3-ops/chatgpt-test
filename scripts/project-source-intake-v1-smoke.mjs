...
assert.equal(context.context.fact_version_refs.some((ref) => ref.fact_id === 'fact-name' && ref.version >= 1), true);
assert.equal(context.context.source_refs.includes('src-owned'), true);
assert.equal(context.context.approved_assets.some((asset) => asset.asset_id === 'asset-project-image'), true);
assert.equal(context.context.rights_constraints.approved_asset_ids.includes('asset-project-image'), true);
assert.equal(context.context.rights_constraints.reference_content_copy_forbidden, true);
assert.equal(Array.isArray(context.context.human_decision_refs), true);
assert.deepEqual(context.context.open_critical_conflicts, []);
