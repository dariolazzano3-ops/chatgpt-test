...
import { cloudflareWorkersAiStagingEvidence, isCloudflareWorkersAiStagingVerified } from '../src/cloudflare-workers-ai-staging-evidence.js';
...
import { posthogStagingEventEvidence, isPostHogStagingAnalyticsVerified } from '../src/posthog-staging-event-evidence.js';
...
const cfAiEvidence = cloudflareWorkersAiStagingEvidence();
assert.equal(isCloudflareWorkersAiStagingVerified(), true);
assert.equal(cfAiEvidence.inference.model, '@cf/zai-org/glm-4.7-flash');
assert.equal(cfAiEvidence.inference.http_status, 200);
assert.equal(cfAiEvidence.inference.api_success, true);
assert.equal(cfAiEvidence.inference.total_tokens, 14);
assert.equal(cfAiEvidence.inference.neurons, 0.2006);
assert.equal(cfAiEvidence.cost_guard.zero_cost_verified, true);
assert.equal(cfAiEvidence.cost_guard.variable_cost_eur, 0);
assert.equal(cfAiEvidence.cost_guard.openai_paid_fallback_used, false);
assert.equal(cfAiEvidence.safety.synthetic_test_data_only, true);
assert.equal(cfAiEvidence.safety.real_customer_data, false);
assert.equal(cfAiEvidence.safety.production_deploy, false);
...
const posthogEvidence = posthogStagingEventEvidence();
assert.equal(isPostHogStagingAnalyticsVerified(), true);
assert.equal(posthogEvidence.delivery.accepted_batch_count, 1);
assert.equal(posthogEvidence.delivery.event_count, 5);
assert.equal(posthogEvidence.delivery.retries_performed, 0);
assert.equal(posthogEvidence.verification.page_view_count, 1);
assert.equal(posthogEvidence.verification.cta_clicked_count, 1);
assert.equal(posthogEvidence.verification.lead_submitted_count, 1);
assert.equal(posthogEvidence.verification.automation_started_count, 1);
assert.equal(posthogEvidence.verification.lead_persisted_count, 1);
assert.equal(posthogEvidence.verification.automation_failed_count, 0);
assert.equal(posthogEvidence.verification.pii_properties_present, false);
assert.equal(posthogEvidence.safety.variable_cost_eur, 0);
assert.equal(posthogEvidence.safety.production_deploy, false);
...
assert.equal(stack.factories.ai.cloudflare_ai_runtime_evidence.inference.http_status, 200);
assert.equal(stack.factories.ai.cloudflare_ai_runtime_evidence.inference.neurons, 0.2006);
assert.equal(stack.factories.ai.cloudflare_ai_runtime_evidence.cost_guard.variable_cost_eur, 0);
assert.equal(stack.factories.ai.cloudflare_ai_blocker, null);
...
assert.equal(stack.factories.business.staging_write_plan.public_anonymous_access, false);
assert.equal(stack.factories.business.staging_write_plan.data_api_exposure, 'not_relied_upon');
...
assert.equal(cfWeb?.activation, 'historical_read_and_staging_deploy_evidence');
...
assert.equal(cfAi?.activation, 'historical_staging_inference_evidence');
assert.equal(cfAi?.historical_workers_ai_models_read, 'permission_missing');
assert.equal(cfAi?.staging_inference_verified, true);
assert.equal(cfAi?.staging_inference_evidence.inference.model, '@cf/zai-org/glm-4.7-flash');
assert.equal(cfAi?.staging_inference_evidence.inference.neurons, 0.2006);
assert.equal(cfAi?.zero_cost_verified, true);
assert.equal(cfAi?.paid_fallback, 'disabled');
...
assert.equal(supabase?.activation, 'historical_read_and_staging_write_evidence');
...
assert.equal(posthog?.staging_analytics_evidence.delivery.event_count, 5);
assert.equal(posthog?.staging_analytics_evidence.verification.automation_failed_count, 0);
...
assert.equal(make?.activation, 'historical_staging_execution_evidence');
...
assert.equal(bakery.activation_status.ai_cloudflare_runtime_verified, true);
...
assert.equal(bakery.activation_status.business_posthog_staging_analytics_verified, true);
...
assert.equal(bakery.activation_evidence.ai_cloudflare_staging_runtime.inference.http_status, 200);
assert.equal(bakery.activation_evidence.ai_cloudflare_staging_runtime.inference.neurons, 0.2006);
assert.equal(bakery.activation_evidence.ai_cloudflare_staging_runtime.cost_guard.variable_cost_eur, 0);
...
assert.equal(bakery.activation_evidence.business_posthog_staging_analytics.delivery.github_actions_run_id, 33287690485);
assert.equal(bakery.activation_evidence.business_posthog_staging_analytics.safety.variable_cost_eur, 0);
...
assert.equal(bakery.next_gate, 'STAGING_EXECUTION_APPROVAL_REQUIRED');
