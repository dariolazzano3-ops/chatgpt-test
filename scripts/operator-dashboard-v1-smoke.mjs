...
assert.match(shell.payload, /function humanFactory\(/);
assert.match(shell.payload, /function humanProviderState\(/);
assert.match(shell.payload, /function humanCostState\(/);
assert.match(shell.payload, /function formatOperatorTimestamp\(/);
assert.match(shell.payload, /function renderStructuredHumanValue\(/);
assert.match(shell.payload, /timeZone:'Europe\/Berlin'/);
assert.match(shell.payload, /year:'numeric'/);
assert.doesNotMatch(shell.payload, /Number\(v\|\|0\)/, 'null money must not become zero in the shared presenter');
