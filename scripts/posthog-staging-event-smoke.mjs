...
const result = await sendPostHogStagingBatchOnce(plan, {
  ...
  resolve_secret: async (ref) => ref === 'secret:POSTHOG_PROJECT_TOKEN' ? 'secret-project-token' : null,
  production_deploy: false
});
...
assert.equal(sent.batch.every((event) => event.properties.distinct_id === schema.distinct_id), true);
...
const rejected = await sendPostHogStagingBatchOnce(plan, {
  ...
  resolve_secret: async () => 'secret-project-token',
  production_deploy: false
});
