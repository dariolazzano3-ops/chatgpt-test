...
const needRegex = (label, text, expected) => expected.test(text) ? passes.push(label) : failures.push(`${label}: expected ${expected}`);
...
needRegex('Job state schema is V2-or-newer', jobs, /version:\s*[2-9]\d*/);
needRegex('Job state telemetry is V1-or-newer', jobs, /telemetry_version:\s*[1-9]\d*/);
...
need('Job state preserves workshop as terminal', jobs, "'WORKSHOP_REQUIRED'");
...
need('QA loop records fulfillment outcomes', loop, "type: 'FULFILLMENT_RESULT'");
...
need('Observability counts workshop terminal jobs', observability, 'jobs_workshop_required');
need('Observability counts QA cache reuse', observability, 'cache_reuses');
need('Observability computes fulfillment pass rate', observability, 'pass_rate_pct');
...
need('Factory Control identifies V3.3-or-newer result', control, 'Factory Control V3.3 completed successfully');
needRegex('Package runtime is V3.3-or-newer generation', pkg, /"version":\s*"1\.(?:[7-9]|[1-9]\d+)\.0-alpha\.1"/);
