...
const jobs = [
  {
    ...
    recovery_reason: 'FAILED_INFRASTRUCTURE',
    events: [
      { type: 'RECOVERY_STARTED', outcome: 'safe_retry' },
...
const snapshot = buildObservabilitySnapshot(jobs, { factory_version: '3.4', generated_at: '2026-08-27T00:00:00.000Z' });
...
assert.equal(snapshot.resilience.recovery_starts, 1);
assert.equal(snapshot.resilience.recovered_jobs, 1);
assert.equal(snapshot.resilience.failure_kinds.project_quality, 1);
assert.equal(snapshot.resilience.recovery_reasons.FAILED_INFRASTRUCTURE, 1);
...
console.log(JSON.stringify({ ok: true, factory_version: snapshot.factory_version, jobs: snapshot.sample.jobs_total, recovered_jobs: snapshot.resilience.recovered_jobs, cache_reuses: snapshot.qa.cache_reuses }, null, 2));
