...
assert.ok(major > 4 || (major === 4 && minor >= 4), `runtime factory_version ${runtime.factory_version} must be >= 4.4`);
...
for (const file of ['src/automation-supervised-runner.js','scripts/automation-supervised-runner-smoke.mjs','docs/LEAN-V4.4.md']) assert.equal(fs.existsSync(file), true, `${file} missing`);
