...
const success = await runMakeSupabaseLeadBridgeOnce(plan, approvals, {
  run_make: async ({ input, scenario_id }) => {
    order.push('make');
    return { ok: true, scenario_id, execution_id: 'make-exec-test', execution_status: 'success', scenario_restored_inactive: true, synthetic_test_data_only: true, synthetic_payload: input };
  },
...
assert.deepEqual(order, ['make','supabase']);
...
console.log('RIOSYSTEMS Make -> Supabase bridge executor smoke: OK');
