...
assert.ok(runtimeMajor > 4 || (runtimeMajor === 4 && runtimeMinor >= 7), `runtime factory_version ${runtime.factory_version} must be >= 4.7`);
...
for (const file of ['src/ai-mission-bridge.js','src/ai-mission-adapter.js','src/ai-executor.js','src/mission-execution-router.js','scripts/ai-mission-bridge-smoke.mjs','scripts/mission-router-ai-smoke.mjs']) assert.equal(fs.existsSync(file), true, `${file} missing`);
