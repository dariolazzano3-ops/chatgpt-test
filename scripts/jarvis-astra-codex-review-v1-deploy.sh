...
printf '%s\n' "$STATUS_BEFORE" | grep -Eq 'DIRTY_LINES=0|DIRTY=0|WORKTREE_CLEAN=(true|yes|1)' \
  || { echo RUNTIME_NOT_PROVEN_CLEAN; exit 20; }

# A failed maintenance attempt can leave its pending bundle behind even after
# the helper has rolled the runtime back. Recover only an unmistakably stale
# bundle from THIS Astra deploy. Anything foreign or currently active remains
# a hard stop.
if [[ -e "$INBOX" ]]; then
  if ps -eo args= | grep -E '^(/usr/bin/)?(sudo )?/?usr/local/sbin/jarvis-maintenance install
...
echo "ASTRA_CODEX_REVIEW_DEPLOY=PASS"
 >/dev/null 2>&1; then
    echo MAINTENANCE_INBOX_BUSY_ACTIVE_INSTALL
    exit 17
  fi

  STALE_STAGE="$(mktemp -d /tmp/jarvis-astra-stale-inbox.XXXXXX)"
  trap 'rm -rf "$STALE_STAGE"' RETURN
  if ! tar -xzf "$INBOX" -C "$STALE_STAGE" manifest.json 2>/dev/null; then
    rm -rf "$STALE_STAGE"
    trap - RETURN
    echo MAINTENANCE_INBOX_BUSY_UNREADABLE
    exit 17
  fi

  if ! python3 - "$STALE_STAGE/manifest.json" "$TARGET_BRANCH" "$RUNTIME_HEAD" <<'PY'
import json,sys
...
PY
  then
    rm -rf "$STALE_STAGE"
    trap - RETURN
    echo MAINTENANCE_INBOX_BUSY_FOREIGN
    exit 17
  fi
