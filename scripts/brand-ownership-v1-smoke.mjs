...
assert.ok(canonical.includes('The canonical parent-brand name is **YSRIO GROUP**'), 'YSRIO GROUP canonical boundary missing');
assert.ok(canonical.includes('must not claim that a legally incorporated multi-company holding or group structure already exists'), 'legal group boundary missing');
assert.ok(canonical.includes('YSRIO GROUP → AURENTARA SYSTEMS → existing RIOSYSTEMS Technical Core'), 'canonical ownership architecture missing');
...
assert.equal(publicProject.brand.public_brand, 'AURENTARA SYSTEMS');
assert.equal(publicProject.brand.parent_brand, 'YSRIO GROUP');
assert.equal(publicProject.brand.endorsement, 'A YSRIO Company');
assert.equal(publicProject.brand.public_domain_reserved, 'aurentarasystems.com');
assert.equal(publicProject.brand.parent_domain_reserved, 'ysrio.com');
...
assert.ok(publicReadme.includes('YSRIO GROUP is the strategic umbrella / ownership brand'), 'public README ownership boundary missing');
assert.ok(publicReadme.includes('A YSRIO Company'), 'approved compact endorsement boundary missing');
...
console.log(JSON.stringify({
  ...
  compact_parent_shorthand: 'YSRIO',
